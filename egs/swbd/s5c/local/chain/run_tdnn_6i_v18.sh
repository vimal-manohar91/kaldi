#!/bin/bash
# _6i_v18 is as _6i_v16 but no stats
# _6i_v16 is as _6i_v3 but use block affine in the first two layers of conv nonlinearity 
# and added stats in 2nd nonlin and used self_repair option.
...
#System                    6i_8l  6i_v3   6v
#WER on train_dev(tg)      17.39  17.05   15.00
#WER on train_dev(fg)      16.16  15.87   13.91
#WER on eval2000(tg)       19.2   19.0   17.2
#WER on eval2000(fg)       17.5   17.3   15.7
#Final train prob     -0.0830084  -0.0924293   -0.120026
#Final valid prob     -0.101803   -0.113381   -0.140628
#Final train prob (xent)      -1.3473 -1.29109 -1.5431
#Final valid prob (xent)      -1.43502  -1.39453 -1.56975
...
dir=exp/chain/tdnn_6i_v18_no_stats # Note: _sp will get added to this if $speed_perturb == true.
...
#egs_dir=exp/chain/tdnn_6h$suffix/egs
#egs_dir=exp/chain/tdnn_6i_v10$suffix/egs
...
if [ $stage -le 12 ]; then
 ...
 steps/nnet3/chain/train_tdnn.sh --stage $train_stage \
    --use-raw-wave-feat true --wav-input $wav_input --max-input-shift 0.2 --add-first-layer-period $add_first_layer_period \
    --shrink $shrink --shrink-threshold $shrink_threshold --component-to-shrink $component_to_shrink \
    --xent-regularize 0.1 --egs-dir $egs_dir \
    --leaky-hmm-coefficient $leak_hmm \
    --l2-regularize $l2reg \
    --conv-opts "--conv-filter-dim 250 --conv-num-filters 100 --conv-filter-step 10 --conv-jesus-stddev-scale 0.1 --conv-param-stddev 0.1 --conv-bias-stddev 0.01 --pnorm-block-dim 1 --conv-self-repair-scale $self_repair_scale --add-log-stddev true" \
    ...
    --splice-indexes "-2,-1,0,1,2 0 0 -1,0,1 -1,0,1,2 -3,0,3 -3,0,3 -3,0,3 -6,-3,0" \
