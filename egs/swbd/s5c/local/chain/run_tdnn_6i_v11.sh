#!/bin/bash
# _6i_v11 is as _6i_v5 but params changed from 500 to 400 
# _6i_v5 is as _6i_v4 but it has mean+stddev+count
...
leak_hmm=0.1
l2reg=0.00005
conv_self_repair_scale=1e-5
dir=exp/chain/tdnn_6i_v11 # Note: _sp will get added to this if $speed_perturb == true.
...
if [ $stage -le 12 ]; then
 ...
 steps/nnet3/chain/train_tdnn.sh --stage $train_stage \
    ...
    --l2-regularize $l2reg \
    --conv-opts "--conv-filter-dim 250 --conv-num-filters 100 --conv-filter-step 5 --conv-param-stddev-scale 0.5 --con-bias-stddev 0.0 --pnorm-block-dim 1 --conv-self-repair-scale $conv_self_repair_scale " \
    --jesus-opts "--jesus-forward-input-dim 500  --jesus-forward-output-dim 1800 --jesus-hidden-dim 7500 --jesus-stddev-scale 0.1 --final-layer-learning-rate-factor 0.25" \
    --splice-indexes "-2,-1,0,1,2 0 0 -1,0,1,mean+stddev+count(-99:1:9:99) -1,0,1,2,mean+stddev+count(-99:1:9:99) -3,0,3,mean+stddev+count(-99:3:9:99) -3,0,3,mean+stddev+count(-99:3:9:99) -3,0,3,mean+stddev+count(-99:3:9:99) -6,-3,0,mean+stddev+count(-99:3:9:99) 0" \
...
if [ $stage -le 14 ]; then
  for decode_set in train_dev eval2000; do
      ...
      steps/nnet3/decode_raw.sh --acwt 1.0 --post-decode-acwt 10.0 \
         --extra-left-context 20 --frames-per-chunk 200 \
          --nj 50 --cmd "$decode_cmd" --wav-input $wav_input \
