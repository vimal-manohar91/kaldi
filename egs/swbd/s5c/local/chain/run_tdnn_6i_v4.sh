#!/bin/bash
# _6i_v4 is as _6i, but it uses leak-hmm=0.1 and it added ivector.
# _6i is as _6h, but adding the mean+stddev features for all hidden layers.
#it uses raw-waveform with convolution in the first layer.
# The improvement is small but consistent (0.1, 0.1, 0.0, 0.1) and also seen
# in the train and valid probs.
# 6i_v3 => no iVector
# 6i_v4 => with iVector
# 6i_v5 => no iVector
#System                  6v  6i_v3(50 f_per_chunk) 6i_v3(200 f_per_chunk) 6i_v5(200 f_per_chunk)  6i_v4(50 f_per_chunk) 6i_v4(200 f_per_chunk) 7b(200 f_per_chunk) 6i_v11
#WER on train_dev(tg)   15.0  17.05 16.15 15.65   16.03  15.47 16.64 15.46
#WER on train_dev(fg)   13.91 15.87 15.08 14.63   14.92  14.37 15.24 14.47
#WER on eval2000(tg)    17.2  19.0 18.1  18.5    18.0  17.7 19.0 18.1
#WER on eval2000(fg)    15.7  17.3 16.3  16.8    16.3  16.1 17.2  16.4
#WER on eval2000(tg,swbd) 10.4  11.4 10.6  11.0    10.8  10.5 11.5  
#Final train prob     -0.120026 -0.0924293 -0.0924293 -0.0864813 -0.0904626 -0.0904626 -0.0985038 -0.0950671
#Final valid prob     -0.140628 -0.113381 -0.113381  -0.105914 -0.112355  -0.112355 -0.113696 -0.11482
#Final train prob (xent)      -1.5431 -1.29109  -1.29109 -1.27436 -1.26079 -1.26079 -1.58229  -1.21569
#Final valid prob (xent)      -1.56975 -1.39453 -1.29109  -1.36036 -1.36459 -1.26079 -1.59966 -1.30129
...
leak_hmm=0.1
l2reg=0.00005
dir=exp/chain/tdnn_6i_v4 # Note: _sp will get added to this if $speed_perturb == true.
...
egs_dir=exp/chain/tdnn_6i_v4$suffix/egs
...
if [ $stage -le 12 ]; then
 ...
 steps/nnet3/chain/train_tdnn.sh --stage $train_stage \
    ...
    --xent-regularize 0.1 \
    --leaky-hmm-coefficient $leak_hmm --egs-dir $egs_dir\
    --l2-regularize $l2reg \
    --conv-opts "--conv-filter-dim 250 --conv-num-filters 100 --conv-filter-step 5 --conv-jesus-stddev-scale 1.0 --pnorm-block-dim 1" \
    ...
    --splice-indexes "-2,-1,0,1,2 0 0 -1,0,1,mean+stddev(-99:1:9:99) -1,0,1,2,mean+stddev(-99:1:9:99) -3,0,3,mean+stddev(-99:3:9:99) -3,0,3,mean+stddev(-99:3:9:99) -3,0,3,mean+stddev(-99:3:9:99) -6,-3,0,mean+stddev(-99:3:9:99)" \
...
if [ $stage -le 13 ]; then
  ...
  # the lang directory.
  utils/mkgraph2.sh --self-loop-scale 1.0 data/lang_sw1_tg $dir $dir/graph_sw1_tg
fi
...
if [ $stage -le 14 ]; then
  ...
  for decode_set in eval2000 rt03; do
      (
      steps/nnet3/decode_raw.sh --acwt 1.0 --post-decode-acwt 10.0 --iter $iter \
