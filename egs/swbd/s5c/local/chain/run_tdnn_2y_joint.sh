...
obj_scales="1.0:0.1" # comma-seprated objective scales used to scale different 
                     # output-node's objective and derivs during training
...
egs_dir=exp/chain/tdnn_2y_joint${suffix}/egs
...
if [ $stage -le 12 ]; then
 ...
 steps/nnet3/chain/train_tdnn_multiple_targets.sh --stage $train_stage \
    ...
    --obj-scales "$obj_scales" \
    data/${train_set}_hires $treedir exp/tri4_ali_nodup$suffix exp/tri4_lats_nodup$suffix $dir  || exit 1;
fi
