...
if [ $# != 3 ]; then
  echo "Usage: $0 <data> <plda-dir> <ivector-dir>"
...
if [ $stage -le 0 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/cluster_ivectors.JOB.log \
    ivector-cluster-plda --ivector-weights-rspecifier=scp:$dir/ivector_weights.JOB.scp \
      --utt2num-rxfilename=ark:$data/utt2num $plda scp:$dir/ivector.JOB.scp \
