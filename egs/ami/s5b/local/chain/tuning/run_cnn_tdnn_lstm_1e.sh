...
finetune_smbr_factor_schedule="output=0.9,0.9"
smbr_leaky_hmm_coefficient=0.00001
finetune_initial_effective_lr=0.000000125
finetune_final_effective_lr=0.000000125
finetune_num_epochs=1
finetune_stage=-2
finetune_affix=_finetune_smbr0.9_mmi0.1_onesil
finetune_dropout_schedule= #'0,0@0.20,0.15@0.50,0'
...
if [ $stage -le 14 ]; then
  ...
  steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
      ...
      --cmd "$train_cmd" 4200 ${lores_train_data_dir} data/lang_chain $lat_dir $tree_dir
fi
...
if [ $stage -le 16 ]; then
 ...
 steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --trainer.optimization.backstitch-training-scale 1.0 \
...
if [ $stage -le 20 ]; then
  mkdir -p ${dir}${finetune_affix}
  cp -r $dir/{frame_subsampling_factor,*opts,tree,final.*,configs,0.trans_mdl,*.fst} $dir${finetune_affix}
  cp $dir/final.mdl ${dir}${finetune_affix}/input.mdl
fi
...
if [ $stage -le 21 ]; then
  steps/nnet3/chain/train.py --stage $finetune_stage \
    --cmd "$decode_cmd" --trainer.input-model=$dir/input.mdl \
    --feat.online-ivector-dir $train_ivector_dir \
    --feat.cmvn-opts "--norm-means=false --norm-vars=false" \
    --chain.xent-regularize $xent_regularize \
    --chain.leaky-hmm-coefficient 0.1 \
    ...
    --egs.chunk-left-context-initial 0 \
    --egs.chunk-right-context-final 0 \
    --trainer.dropout-schedule="$finetune_dropout_schedule" \
    --trainer.num-chunk-per-minibatch 64,32 \
    --trainer.frames-per-iter 1500000 \
    ...
    --trainer.optimization.num-jobs-initial 2 \
    --trainer.optimization.num-jobs-final 12 \
    --trainer.optimization.initial-effective-lrate $finetune_initial_effective_lr \
    --trainer.optimization.final-effective-lrate $finetune_final_effective_lr \
    --trainer.optimization.backstitch-training-scale 1.0 \
    --trainer.optimization.backstitch-training-interval 4 \
    --trainer.max-param-change 2.0 \
    --trainer.deriv-truncate-margin 8 \
    ...
    --tree-dir $tree_dir \
    --lat-dir $lat_dir \
    --dir $dir --lang data/lang_${LM} || exit 1
fi
