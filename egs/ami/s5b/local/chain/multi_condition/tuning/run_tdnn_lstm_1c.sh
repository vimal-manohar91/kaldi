...
original_lat_dir=${src_dir}_${train_set}_sp_lats_ihmdata
lat_dir=${src_dir}_${train_set}_sp${rvb_affix}_lats_ihmdata
...
for f in ${teacher_data_dir}_hires/feats.scp ${teacher_ivector_extractor}/final.ie \
  ...
  $student_data_dir/feats.scp $student_ivector_dir/ivector_online.scp; do
...
if [ $stage -le 13 ]; then
  ...
  steps/nnet3/align_lats.sh \
    ...
    --nj 100 --cmd "$train_cmd" \
    ${teacher_data_dir}_hires \
    data/lang $src_dir $original_lat_dir
...
if [ $stage -le 16 ]; then
 ...
 steps/nnet3/chain/train_ts.py --stage $train_stage \
    ...
    --egs.opts "--frames-overlap-per-eg 0 --acwt 1.0" \
