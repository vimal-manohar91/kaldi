...
# This script is based on local/chain/tuning/run_cnn_tdnn_lstm_1c.sh, but
...
# local/chain/multi_condition/run_cnn_tdnn_lstm.sh --mic ihm --train-set train_cleaned --gmm tri3_cleaned &
# local/chain/multi_condition/run_cnn_tdnn_lstm.sh --mic sdm1 --use-ihm-ali true --train-set train_cleaned --gmm tri3_cleaned &
# local/chain/multi_condition/run_cnn_tdnn_lstm.sh --mic mdm8 --use-ihm-ali true --train-set train_cleaned --gmm tri3_cleaned &
...
if $use_ihm_ali; then
  ...
  dir=exp/$mic/chain${nnet3_affix}${rvb_affix}/cnn_tdnn_lstm${tlstm_affix}_sp${rvb_affix}_bi_ihmali
  ...
  dir=exp/$mic/chain${nnet3_affix}${rvb_affix}/cnn_tdnn_lstm${tlstm_affix}_sp${rvb_affix}_bi
fi
...
if [ $stage -le 13 ]; then
  ...
  local/copy_rvb_lattices.sh --cmd "$train_cmd" --nj 100 --num-data-reps $num_data_reps \
    $train_data_dir $original_lat_dir $lat_dir_ihmdata \
    $lat_dir || exit 1
fi
...
if $test_online_decoding && [ $stage -le 19 ]; then
  steps/online/nnet3/prepare_online_decoding.sh \
    ...
    data/lang_${LM} exp/$mic/nnet3${nnet3_affix}${rvb_affix}/extractor ${dir} ${dir}_online
