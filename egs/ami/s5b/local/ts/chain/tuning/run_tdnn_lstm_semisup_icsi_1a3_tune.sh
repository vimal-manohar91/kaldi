...
if [ -z "$unsup_src_lat_dir_rvb" ]; then
...
if [ $stage -le 8 ]; then
  ...
  tdnn_opts="l2-regularize=$tdnn_l2reg"

  if [ $lstmp_layer == "fast-lstmp-layer" ]; then
    lstm_opts="l2-regularize=$lstm_l2reg decay-time=$decay_time"
  else
    lstm_opts="decay-time=$decay_time"
  fi

  output_opts="l2-regularize=$output_l2reg"
  ...
  relu-$norm-layer name=tdnn1 dim=$hidden_dim $tdnn_opts
  relu-$norm-layer name=tdnn2 input=Append(-1,0,1) dim=$hidden_dim $tdnn_opts
  relu-$norm-layer name=tdnn3 input=Append(-1,0,1) dim=$hidden_dim $tdnn_opts

  # check steps/libs/nnet3/xconfig/lstm.py for the other options and defaults
  $lstmp_layer name=lstm1 cell-dim=$cell_dim recurrent-projection-dim=$projection_dim non-recurrent-projection-dim=$projection_dim delay=-3 $lstm_opts
  relu-$norm-layer name=tdnn4 input=Append(-3,0,3) dim=$hidden_dim $tdnn_opts
  relu-$norm-layer name=tdnn5 input=Append(-3,0,3) dim=$hidden_dim $tdnn_opts
  relu-$norm-layer name=tdnn6 input=Append(-3,0,3) dim=$hidden_dim $tdnn_opts
  $lstmp_layer name=lstm2 cell-dim=$cell_dim recurrent-projection-dim=$projection_dim non-recurrent-projection-dim=$projection_dim delay=-3 $lstm_opts
  relu-$norm-layer name=tdnn7 input=Append(-3,0,3) dim=$hidden_dim $tdnn_opts
  relu-$norm-layer name=tdnn8 input=Append(-3,0,3) dim=$hidden_dim $tdnn_opts
  relu-$norm-layer name=tdnn9 input=Append(-3,0,3) dim=$hidden_dim $tdnn_opts
  $lstmp_layer name=lstm3 cell-dim=$cell_dim recurrent-projection-dim=$projection_dim non-recurrent-projection-dim=$projection_dim delay=-3 $lstm_opts
...
if [ $stage -le 14 ]; then
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --chain.l2-regularize $chain_l2reg \
    ...
    --trainer.optimization.shrink-value $proportional_shrink \
