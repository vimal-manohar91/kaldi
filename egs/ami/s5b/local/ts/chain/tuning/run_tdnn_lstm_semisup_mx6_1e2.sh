...
supervision_weights=1,1,1,1   # Supervision weights: AMI, Mixer 6 (headset), Mixer 6 (array)
num_copies=3,3,3,1    # Make copies of data: AMI, Mixer 6 (headset), Mixer 6 (array)
...
if [ $stage -le 9 ]; then
  ...
  learning_rate_factor=$(echo "print 1.0/$xent_regularize" | python)
  ...
  output-layer name=output input=lstm3 output-delay=$label_delay include-log-softmax=false dim=$num_targets max-change=1.5 $output_opts learning-rate-factor=2.0
  output-layer name=output-1 input=lstm3 output-delay=$label_delay include-log-softmax=false dim=$num_targets max-change=1.5 $output_opts learning-rate-factor=2.0
...
if [ $stage -le 13 ]; then
  steps/nnet3/chain/multilingual/combine_egs.sh --cmd "$train_cmd" \
    ...
    --affixes "-1 -2 -1 -1" \
    4 $sup_egs_dir $sup_egs_dir $unsup_src_egs_dir $unsup_tgt_egs_dir \
    $dir/egs_comb
fi
