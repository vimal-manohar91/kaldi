...
# This script is similar to run_tdnn_lstm_semisup_mx6_1d.sh in using
# separate denominator FST for unsupervised data (Mixer 6), but 
# uses two output layers -- one trained on only supervised data,
# and the other trained on both supervised and unsupervised data.
...
num_copies=6,3,3,1    # Make copies of data: AMI, Mixer 6 (headset), Mixer 6 (array)
