...
if [ $stage -le 0 ]; then
  ...
  gunzip -c db/TEDLIUM_release-3/LM/*.en.gz | sed 's/ <\/s>//g' | gzip -c > ${dir}/data/train_all.txt.gz

  gunzip -c ${dir}/data/train_all.txt.gz | head -n $num_dev_sentences > ${dir}/data/text/dev.txt
  gunzip -c ${dir}/data/train_all.txt.gz | tail -n +$[num_dev_sentences] > ${dir}/data/text/train.txt

  ## use a subset of the annotated training data as the dev set .
  ## Note: the name 'dev' is treated specially by pocolm, it automatically
  ## becomes the dev set.
  #head -n $num_dev_sentences < data/tedlium_train/text | cut -d " " -f 2-  > ${dir}/data/text/dev.txt
  ## .. and the rest of the training data as an additional data source.
  ## we can later fold the dev data into this.
  #tail -n +$[$num_dev_sentences+1] < data/tedlium_train/text | cut -d " " -f 2- >  ${dir}/data/text/ted.txt
  ...
  cut -d " " -f 2-  < data/tedlium_dev/text  > ${dir}/data/real_dev_set.txt
