...
# This script generates numerator posterior after splitting the egs.
# This script is similar to _a, but uses different parameters of num-copies 
# lm_weights
# num_copies=10,10,3,3
# lm_weights=2,1   # src, tgt weight
...
ivector_train_data_dir=data/train_ihm_sdm_icsi_sp_rvb_hires
...
if [ $stage -le 4 ]; then
  ...
  local/nnet3/run_train_ivector.sh --exp-root $exp_root \
    --nnet3-affix "${nnet3_affix}_ihm_sdm_icsi" \
...
if [ -z "$sup_src_egs_dir" ]; then
  ...
  if [ $stage -le 9 ]; then
    ...
    steps/nnet3/chain/get_egs.sh --cmd "$decode_cmd" \
      ...
      --cmvn-opts "$cmvn_opts" \
      --online-ivector-dir $exp_root/nnet3${nnet3_affix}_ihm_sdm_icsi/ivectors_train_ihm_sdm_icsi_sp_rvb_hires \
...
if [ $stage -le 14 ]; then
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --feat.online-ivector-dir $exp_root/nnet3${nnet3_affix}_ihm_sdm_icsi/ivectors_train_ihm_sdm_icsi_sp_rvb_hires \
...
ivector_root_dir=$exp_root/nnet3${nnet3_affix}_ihm_sdm_icsi
