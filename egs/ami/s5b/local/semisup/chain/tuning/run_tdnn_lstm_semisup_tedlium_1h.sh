...
unsup_src_ivector_dir=$(dirname $src_ivector_extractor)/ivectors_${unsup_dataset}_sp
...
################################################################################
# Train i-vector extractor using unsupervised data
################################################################################

if [ $stage -le -1 ]; then
  local/nnet3/run_ivector.sh --nnet3-affix $nnet3_affix \
    $sup_data_dir $unsup_data_dir_sp || exit 1
fi

ivector_extractor=exp/$mic/nnet3${nnet3_affix}/extractor
unsup_ivector_dir=exp/$mic/nnet3${nnet3_affix}/ivectors_${unsup_dataset}_sp_hires
sup_ivector_dir=exp/$mic/nnet3${nnet3_affix}/ivectors_$(basename $sup_data_dir)
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 11 ]; then
    ...
    if [ "$lattice_prune_beam" == "0.0" || "$lattice_prune_beam" == "0" ]; then
      get_egs_script=steps/nnet3/chain/get_egs.sh
    fi
