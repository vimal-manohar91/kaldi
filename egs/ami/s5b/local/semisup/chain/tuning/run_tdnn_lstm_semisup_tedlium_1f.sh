...
src_treedir=exp/ihm/chain_cleaned/tree_bi
...
supervision_weights=1,1   # Supervision weights: AMI, Tedlium
num_copies=2,1    # Make copies of data: AMI, Tedlium
...
unsup_lat_dir=$src_dir/decode${unsup_graph_affix}_${unsup_dataset}_sp   # training lattices directory

dir=$exp_root/chain${chain_affix}/tdnn_lstm${tdnn_affix}_sp

lang=data/lang_chain

for f in $ivector_extractor/final.ie $src_treedir/final.mdl $src_dir/final.mdl; do
  if [ ! -f $f ]; then
...
utils/lang/check_phones_compatible.sh $src_treedir/phones.txt data/lang/phones.txt || exit 1
...
diff $src_treedir/tree $src_dir/tree || exit 1
...
if [ $stage -le -2 ]; then
  steps/nnet3/decode_semisup.sh --nj $nj --cmd "$decode_cmd" \
    ...
    --skip-scoring true \
    $unsup_graph_dir ${unsup_data_dir_sp} $unsup_lat_dir || exit 1
fi
...
unsup_best_path_dir=$src_dir/best_path${unsup_graph_affix}_${unsup_dataset}_sp

if [ $stage -le 5 ]; then
  steps/best_path_weights.sh --cmd "$decode_cmd" \
    ${unsup_data_dir} ${unsup_lat_dir} \
    $src_dir/best_path${unsup_graph_affix}_${unsup_dataset}_sp
fi

mkdir -p $dir
deriv_weights_scp=$unsup_lat_dir/weights.scp
...
if [ $stage -le 6 ]; then
  ...
  ) | sort -k1,1 > $unsup_lat_dir/weights.scp
fi

unsup_tree_dir=${src_dir}/tree${unsup_graph_affix}_${unsup_dataset}_sp
...
if [ $stage -le 7 ]; then
  steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
      ...
      --cmd "$train_cmd" 4200 ${unsup_data_dir_sp} data/lang_chain \
      $src_dir/best_path${unsup_graph_affix}_${unsup_dataset}_sp \
      $unsup_tree_dir
fi
...
if [ $stage -le 8 ]; then
  steps/nnet3/chain/make_weighted_den_fst.sh \
    --cmd "$train_cmd" \
    $unsup_tree_dir $dir/unsup_den_fst
fi

if [ $stage -le 10 ]; then
  echo "$0: creating neural net configs using the xconfig parser";

  num_targets_sup=$(tree-info $src_treedir/tree |grep num-pdfs|awk '{print $2}')
  num_targets_unsup=$(tree-info $unsup_tree_dir/tree | grep num-pdfs|awk '{print $2}')
  ...
  output-layer name=output input=lstm3 output-delay=$label_delay include-log-softmax=false dim=$num_targets_sup max-change=1.5 $output_opts
  output-layer name=output-1 input=lstm3 output-delay=$label_delay include-log-softmax=false dim=$num_targets_unsup max-change=1.5 $output_opts
  ...
  output-layer name=output-xent input=lstm3 output-delay=$label_delay dim=$num_targets_sup learning-rate-factor=$learning_rate_factor max-change=1.5 $output_opts
  output-layer name=output-1-xent input=lstm3 output-delay=$label_delay dim=$num_targets_unsup learning-rate-factor=$learning_rate_factor max-change=1.5 $output_opts
...
if [ -z "$sup_egs_dir" ]; then
  ...
  if [ $stage -le 10 ]; then
    ...
    steps/nnet3/chain/get_egs.sh --cmd "$decode_cmd" \
      ...
      $sup_data_dir $src_dir $sup_lat_dir $sup_egs_dir
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 11 ]; then
    ...
    if $use_best_path_words; then
      get_egs_script=steps/nnet3/chain/get_egs.sh
      unsup_egs_opts="$unsup_egs_opts --use-best-path-words $use_best_path_words"
    fi
    ...
    $get_egs_script --cmd "$decode_cmd" --alignment-subsampling-factor 1 \
      ...
      ${unsup_data_dir_sp} $dir/unsup_den_fst \
...
if [ $stage -le 16 ]; then
  for data in dev eval; do
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
      data/$mic/${data}_hires $ivector_extractor \
...
if $decode_tedlium; then
  ...
  if [ $stage -le 19 ]; then
    ...
    # far as the 'topo' is concerned, but this script doesn't read the 'topo' from
    # the lang directory.
    utils/mkgraph.sh --self-loop-scale 1.0 ${test_lang} $unsup_tree_dir $graph_dir
  fi

  nnet3-copy --edits="remove-output-nodes name=output;rename-node old-name=output-1 new-name=output" $dir/final.mdl - | \
    nnet3-am-init $unsup_tree_dir/tree data/lang_chain/topo - $dir/final_output_1.mdl || exit 1
  if [ $stage -le 20 ]; then
    rm -f $dir/.error
