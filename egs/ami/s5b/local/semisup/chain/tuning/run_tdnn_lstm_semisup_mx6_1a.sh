...
src_ivector_extractor=exp/ihm/nnet3_cleaned_rvb/extractor
sup_src_data_dir=data/ihm/train_cleaned_sp_rvb_hires

# Lattices for supervised clean data
sup_src_lat_dir=exp/ihm/chain_cleaned_rvb/tri3_cleaned_train_cleaned_sp_rvb_lats

sup_tgt_data_dir_ihmdata=data/sdm1/train_cleaned_ihmdata_sp_hires
# Lattices for supervised clean data to be used for supervised noisy data
sup_tgt_lat_dir_ihmdata=exp/sdm1/chain_cleaned/tri3_cleaned_train_cleaned_sp_lats_ihmdata

# Supervised noisy data
sup_tgt_data_dir=data/sdm1/train_cleaned_for_combine_sp_hires

# Unsupervised clean and noisy data
unsup_src_data_dir=data/train_mixer6_mic_02_1a_seg
unsup_tgt_data_dir=data/train_mixer6_mic_04_to_13_4k_1a_seg
...
lm_weights=1,1   # src, tgt weight

tdnn_affix=_1a
chain_affix=_semisup_ami_mixer6
nnet3_affix=_semisup_ami_mixer6

kl_factor_schedule="output-0=0,0 output-1=0,0 output-2=0,0 output-3=0,0"
mmi_factor_schedule="output-0=1,1 output-1=1,1 output-2=1,1 output-3=1,1"
...
################################################################################
unsup_src_data_dir_perturbed=${unsup_src_data_dir}_sp_rvb3_hires
...
if [ $stage -le 1 ]; then
  local/semisup/copy_lat_dir.sh --write-compact false \
    --cmd "$decode_cmd" --nj $nj --num-copies 3 \
...
if [ $stage -le 3 ]; then
  local/semisup/copy_lat_dir_mx6_parallel.sh --write-compact false \
    --cmd "$decode_cmd" --nj $nj --num-copies 3 \
...
ivector_train_data_dir=data/train_ihm_sdm_mx6_sp_rvb_hires

if [ $stage -le 4 ]; then
  utils/combine_data.sh \
    $ivector_train_data_dir \
    $unsup_src_data_dir_perturbed $unsup_tgt_data_dir_perturbed \
    $sup_src_data_dir \
    $sup_tgt_data_dir

  local/nnet3/run_train_ivector.sh --exp-root $exp_root \
    --nnet3-affix "${nnet3_affix}_ihm_sdm_mx6" \
    --train-data-dir ${ivector_train_data_dir}
fi
...
if [ $stage -le 8 ]; then
  ...
  output name=output-3 input=output.affine@$label_delay 
  ...
  output name=output-3-xent input=output-xent.log-softmax@$label_delay 
EOF
...
sup_src_dataset=$(basename $sup_src_data_dir)

if [ -z "$sup_src_egs_dir" ]; then
  sup_src_egs_dir=$dir/egs_${sup_src_dataset}
  frames_per_eg=$(cat $src_dir/egs/info/frames_per_eg)

  if [ $stage -le 9 ]; then
    if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $dir/egs/storage ]; then
      utils/create_split_dir.pl \
       /export/b0{5,6,7,8}/$USER/kaldi-data/egs/aspire-$(date +'%m_%d_%H_%M')/s5c/$sup_src_egs_dir/storage $sup_src_egs_dir/storage
    fi
    ...
    steps/nnet3/chain/get_egs.sh --cmd "$decode_cmd" \
      ...
      --cmvn-opts "$cmvn_opts" \
      --online-ivector-dir $exp_root/nnet3${nnet3_affix}_ihm_sdm_mx6/ivectors_train_ihm_sdm_mx6_sp_rvb_hires \
      --generate-egs-scp true \
      $sup_src_data_dir $dir $sup_src_lat_dir $sup_src_egs_dir
    touch $sup_src_egs_dir/.nodelete # keep egs around when that run dies.
  fi
else
  frames_per_eg=$(cat $sup_src_egs_dir/info/frames_per_eg)
fi

sup_tgt_dataset=$(basename $sup_tgt_data_dir)

if [ -z "$sup_tgt_egs_dir" ]; then
  sup_tgt_egs_dir=$dir/egs_${sup_tgt_dataset}
  frames_per_eg=$(cat $sup_src_egs_dir/info/frames_per_eg)

  if [ $stage -le 10 ]; then
    if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $dir/egs/storage ]; then
      utils/create_split_dir.pl \
       /export/b0{5,6,7,8}/$USER/kaldi-data/egs/aspire-$(date +'%m_%d_%H_%M')/s5c/$sup_tgt_egs_dir/storage $sup_tgt_egs_dir/storage
    fi
    ...
    steps/nnet3/chain/get_egs.sh --cmd "$decode_cmd" \
      ...
      --online-ivector-dir $exp_root/nnet3${nnet3_affix}_ihm_sdm_mx6/ivectors_train_ihm_sdm_mx6_sp_rvb_hires \
      --generate-egs-scp true \
      $sup_tgt_data_dir $dir $sup_tgt_lat_dir $sup_tgt_egs_dir
    touch $sup_tgt_egs_dir/.nodelete # keep egs around when that run dies.
  fi
else
  frames_per_eg=$(cat $sup_tgt_egs_dir/info/frames_per_eg)
fi

unsup_egs_opts="$unsup_egs_opts --deriv-weights-scp $deriv_weights_scp --add-numerator-post true"

if [ -z "$unsup_src_egs_dir" ]; then
  unsup_src_egs_dir=$dir/egs_${unsup_src_dataset}_sp_rvb3
...
if [ $stage -le 13 ]; then
  steps/nnet3/chain/multilingual/combine_egs.sh --cmd "$train_cmd" \
    ...
    4 $sup_src_egs_dir $sup_tgt_egs_dir $unsup_src_egs_dir $unsup_tgt_egs_dir \
    $dir/egs_comb
fi
...
if [ $stage -le 14 ]; then
  steps/nnet3/chain/train.py --stage $train_stage \
    --cmd "$train_cmd --mem 4G" --train-queue-opt "--h-rt 00:15:00" --combine-queue-opt "--h-rt 00:59:00" \
    --feat.online-ivector-dir $exp_root/nnet3${nnet3_affix}_ihm_sdm_mx6/ivectors_train_ihm_sdm_mx6_sp_rvb_hires \
    ...
    --feat-dir $sup_src_data_dir \
    --tree-dir $treedir \
    --lat-dir $sup_src_lat_dir \
...
ivector_root_dir=$exp_root/nnet3${nnet3_affix}_ihm_sdm_mx6
...
if [ $stage -le 17 ]; then
  ...
  for dset in dev eval; do
    (
      if [ $stage -le 16 ]; then
        steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
          data/sdm1/${dset}_hires $ivector_root_dir/extractor \
          $ivector_root_dir/ivectors_${dset}_hires
      fi
      ...
      steps/nnet3/decode.sh --nj $nj --cmd "$decode_cmd" \
        ...
        --frames-per-chunk 160 \
        --online-ivector-dir $ivector_root_dir/ivectors_${dset}_hires \
