...
# unsupervised data from Mixer 6 microphone corpus to improve ASR. 
...
unsup_data_dir=data/train_icsi_ihm
...
if [ $stage -le 4 ]; then
  utils/combine_data.sh \
    ${unsup_data_dir}_rvb_hires \
    $unsup_data_dir_perturbed ${unsup_data_dir_sp}

  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    ${unsup_data_dir}_rvb_hires \
    ${unsup_data_dir}_rvb_hires_max2
...
if [ $stage -le 9 ]; then
  ...
  $train_cmd $dir/log/generate_input_mdl.log \
    nnet3-copy --edits="set-learning-rate-factor name=* learning-rate-factor=$primary_lr_factor; set-learning-rate-factor name=output* learning-rate-factor=1.0" \
    $src_dir/final.mdl $dir/input.raw || exit 1;

  cp -rT $src_dir/configs $dir/configs
fi
...
if [ -z "$unsup_egs_dir" ]; then
  unsup_egs_dir=$dir/egs_${unsup_dataset}_sp_rvb1
  if [ $stage -le 11 ]; then
    ...
    steps/nnet3/chain/get_egs_split.sh --cmd "$decode_cmd" --alignment-subsampling-factor 1 \
      ...
      $unsup_data_dir_perturbed $dir/unsup_den_fst \
...
if [ $stage -le 14 ]; then
  cp $unsup_egs_dir/0.trans_mdl $dir/
  cp $unsup_egs_dir/den.fst $dir/
  cp $unsup_egs_dir/tree $dir/
  ...
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --egs.chunk-width $unsup_frames_per_eg \
  ...
  for dset in dev eval; do
    (
      ...
      steps/nnet3/decode.sh --nj $nj --cmd "$decode_cmd" \
        ...
        $graph_dir data/ihm/${dset}_hires $decode_dir || { echo "Failed decoding in $decode_dir"; touch $dir/.error; }
    ) &
  done
  wait
...
if [ $stage -le 18 ]; then
  ...
  for data in dev_icsi eval_icsi; do
    utils/copy_data_dir.sh data/ihm/${data}{,_hires}
    steps/make_mfcc.sh --cmd "$train_cmd" --nj $nj --mfcc-config conf/mfcc_hires.conf data/ihm/${data}_hires
    steps/compute_cmvn_stats.sh data/ihm/${data}_hires
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj "$nj" \
      data/ihm/${data}_hires $ivector_extractor \
