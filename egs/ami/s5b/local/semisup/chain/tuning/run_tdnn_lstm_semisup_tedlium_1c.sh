...
unsup_ivector_dir=$(dirname $ivector_extractor)/ivectors_${unsup_dataset}_sp
...
mkdir -p $dir
deriv_weights_scp=$src_dir/best_path${unsup_graph_affix}_${unsup_dataset}_sp/weights.scp
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 11 ]; then
    ...
    $get_egs_script --cmd "$decode_cmd" --alignment-subsampling-factor 1 \
      ...
      ${kl_fst_scale:+--kl-latdir $unsup_lat_dir --kl-fst-scale $kl_fst_scale} \
...
if [ $extra_scale != 0.0 ]; then
  egs_copy_opts="--apply-output-scale=true"
fi
...
if [ $stage -le 16 ]; then
  for data in dev eval; do
    if [ ! -s $(dirname $ivector_extractor)/ivectors_${data}_hires/ivector_online.scp ]; then
