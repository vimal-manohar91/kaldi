...
extra_ivector_data_dir=
reverberate_extra=false
...
ivector_data_dir=data/${mic}/${train_set}_sp${rvb_affix}_hires
if [ -z "$extra_ivector_data_dir" ]; then
  if $reverberate_extra; then
    if [ $stage -le 3 ]; then
      rvb_opts=()
      rvb_opts+=(--rir-set-parameters "0.5, RIRS_NOISES/simulated_rirs/smallroom/rir_list")
      rvb_opts+=(--rir-set-parameters "0.5, RIRS_NOISES/simulated_rirs/mediumroom/rir_list")
      ...
      python steps/data/reverberate_data_dir.py \
        ...
        --max-noises-per-minute 1 \
        --source-sampling-rate $sample_rate \
        ${extra_ivector_data_dir} ${extra_ivector_data_dir}${rvb_affix}${num_data_reps}

      utils/copy_data_dir.sh ${extra_ivector_data_dir}${rvb_affix}${num_data_reps}{,_hires}
      utils/data/perturb_data_dir_volume.sh ${extra_ivector_data_dir}${rvb_affix}${num_data_reps}_hires
      
      steps/make_mfcc.sh --nj $nj --mfcc-config conf/mfcc_hires.conf \
        --cmd "$train_cmd --max-jobs-run $max_jobs_run" ${extra_ivector_data_dir}${rvb_affix}${num_data_reps}_hires
      steps/compute_cmvn_stats.sh ${extra_ivector_data_dir}${rvb_affix}${num_data_reps}_hires
      utils/fix_data_dir.sh ${extra_ivector_data_dir}${rvb_affix}${num_data_reps}_hires

      utils/combine_data.sh ${extra_ivector_data_dir}${rvb_affix}${num_data_reps}_combine_hires \
        $extra_ivector_data_dir${rvb_affix}${num_data_reps}_hires data/${mic}/${train_set}_sp${rvb_affix}_hires
    fi
    ivector_data_dir=${extra_ivector_data_dir}${rvb_affix}${num_data_reps}_combine_hires
  else
    if [ $stage -le 3 ]; then
      utils/combine_data.sh ${extra_ivector_data_dir}_combine_hires \
        $extra_ivector_data_dir data/${mic}/${train_set}_sp${rvb_affix}_hires
    fi
    ivector_data_dir=${extra_ivector_data_dir}_combine_hires
  fi
fi
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    $ivector_data_dir exp/$mic/nnet3${nnet3_affix}/diag_ubm \
    exp/$mic/nnet3${nnet3_affix}/extractor || exit 1;
fi
