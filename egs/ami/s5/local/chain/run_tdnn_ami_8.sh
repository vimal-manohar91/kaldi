...
wc=`python -c "print ('%.02f' % (float($max_wer) / 100))"`
suffix=cleanup_wer_l$wc

train_set=train
latgen_train_set=train
if [ $use_ihm_ali == "true" ]; then
  latgen_train_set=train_parallel
fi

if [ $stage -le 12 ]; then
  mkdir -p $bad_utts_dir
  steps/cleanup/find_bad_utts.sh --cmd "$decode_cmd" --nj 150 --num-threads 4 \
    data/$mic/${train_set} data/lang $ali_dir $bad_utts_dir
fi

if [ $stage -le 13 ]; then
  lmwt=10
  steps/get_ctm.sh --cmd "$decode_cmd" --use-segments true \
    --print-silence true \
    data/$mic/${train_set} data/lang $bad_utts_dir/lattice_oracle

  grep -v "<eps>" $bad_utts_dir/lattice_oracle/score_$lmwt/train.ctm > \
    $bad_utts_dir/lattice_oracle/score_$lmwt/train.ctm.nosil

  align-text --special-symbol="***" ark:$bad_utts_dir/text \
    ark:$bad_utts_dir/aligned_ref.txt ark:$bad_utts_dir/aligned_text

  mkdir -p $bad_utts_dir/$suffix

  steps/cleanup/create_segments_from_ctm.pl --special-symbol "***" \
    --aligned-ctm-filename $bad_utts_dir/lattice_oracle/score_$lmwt/train.ctm.aligned \
    --wer-cutoff $max_wer --min-seg-len $min_seg_len \
    $bad_utts_dir/lattice_oracle/score_$lmwt/train.ctm.nosil \
    $bad_utts_dir/aligned_text \
    $bad_utts_dir/${suffix}/segments $bad_utts_dir/${suffix}/text
  
  steps/cleanup/resegment_data_dir.sh data/$mic/$train_set \
    $bad_utts_dir/${suffix}/segments $bad_utts_dir/${suffix}/text \
    data/$mic/${train_set}_${suffix}
  utils/fix_data_dir.sh data/$mic/${train_set}_${suffix}
  
  if [ $latgen_train_set != $train_set ]; then
    steps/cleanup/resegment_data_dir.sh data/$mic/${latgen_train_set} \
      $bad_utts_dir/cleanup/segments $bad_utts_dir/cleanup/text \
      data/$mic/${train_set}_${suffix}
    utils/fix_data_dir.sh data/$mic/${latgen_train_set}_${suffix}
  fi
fi

if $speed_perturb; then
  if [ $stage -le 14 ]; then
    local/nnet3/prepare_perturbed_datadir.sh --train-set ${train_set}_${suffix} --mic $mic --nj 100
  fi
  train_set=${train_set}_${suffix}_sp
  latgen_train_set=${latgen_train_set}_${suffix}_sp
else
  train_set=${train_set}_${suffix}
  latgen_train_set=${latgen_train_set}_${suffix}
fi

if [ $stage -le 15 ]; then
  utils/copy_data_dir.sh data/$mic/${train_set} data/$mic/${train_set}_hires
  steps/make_mfcc.sh --mfcc-config conf/mfcc_hires.conf --nj 100 \
    data/$mic/${train_set}_hires exp/$mic/make_hires mfcc_${mic}_hires
  steps/compute_cmvn_stats.sh data/$mic/${train_set}_hires exp/$mic/make_hires mfcc_${mic}_hires
  utils/fix_data_dir.sh data/$mic/${train_set}_hires
fi
...
if [ $stage -le 16 ]; then
  ...
  steps/online/nnet2/copy_data_dir.sh --utts-per-spk-max 2 \
    data/$mic/${train_set}_hires data/$mic/${train_set}_hires_max2
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
    data/$mic/${train_set}_hires_max2 \
    exp/$mic/nnet3/extractor \
    exp/$mic/nnet3/ivectors_${train_set} || exit 1;
fi

ali_dir=${ali_dir}_$suffix
lat_dir=${lat_dir}_$suffix
dir=${dir}_$suffix
