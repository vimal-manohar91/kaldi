...
# This script adds filtering based on clean alignment FER
...
if [ ! -z $max_fer ]; then
  if [ "$use_ihm_ali" == true ]; then
    echo "$0: --max-fer does not make sense when using clean alignments"
    exit 1
  fi
    
  bad_utts_dir=${gmm_dir}_${train_set}_bad_utts_${mic}_cleanali
  if [ $stage -le 12 ]; then
    mkdir -p $bad_utts_dir

    false && { \
      local/nnet3/prepare_parallel_perturbed_alignments.sh --mic $mic \
      --new-mic ${mic}_cleanali \
      --use-sat-alignments true --nj 100; \
    }

    parallel_mic=${mic}_cleanali
    parallel_ali_dir=exp/ihm/${gmm}_${parallel_mic}_train_parallel_sp_ali
  
    for n in `seq $(cat $ali_dir/num_jobs)`; do
      gunzip -c $ali_dir/ali.$n.gz
    done | copy-int-vector ark:- ark,scp:$bad_utts_dir/ali.ark,$bad_utts_dir/ali.scp

    for n in `seq $(cat $parallel_ali_dir/num_jobs)`; do
      gunzip -c $parallel_ali_dir/ali.$n.gz
    done | copy-int-vector ark:- ark,scp:$bad_utts_dir/cleanali.ark,$bad_utts_dir/cleanali.scp

    $train_cmd JOB=1:100 $bad_utts_dir/log/compute_fer_info.JOB.log \
      compute-wer --verbose=2 --mode=present \
      "ark,t:utils/split_scp.pl -j 100 \$[JOB-1] $bad_utts_dir/cleanali.scp | ali-to-phones --per-frame=true $parallel_ali_dir/final.mdl scp:- ark,t:- |" \
      "ark,s,cs,t:utils/split_scp.pl -j 100 \$[JOB-1]  $bad_utts_dir/ali.scp | ali-to-phones --per-frame=true $ali_dir/final.mdl scp:- ark,t:- |" \
      '2>' $bad_utts_dir/fer_info.JOB.txt '>' /dev/null
   

    for n in `seq 100`; do
      awk '/%WER/ {print $2,$3}' $bad_utts_dir/fer_info.$n.txt 
    done | sort -k2,2 >  $bad_utts_dir/fer_${mic}

    awk -v max_fer=$max_fer '{if ($2 < max_fer) print $1}' $bad_utts_dir/fer_${mic} > $bad_utts_dir/utts_fer_l$max_fer
  fi

  if [ $stage -le 13 ]; then
    utils/subset_data_dir.sh --utt-list $bad_utts_dir/utts_fer_l$max_fer \
      data/$mic/${train_set}_hires data/$mic/${train_set}_fer_l${max_fer}_hires
    utils/subset_data_dir.sh --utt-list $bad_utts_dir/utts_fer_l$max_fer \
      data/$mic/${train_set} data/$mic/${train_set}_fer_l${max_fer}
  fi

  suffix=fer_l$max_fer
