...
if [ $stage -le 13 ]; then
  ...
  python utils/append_edits_to_ctm.pl --special-symbol="***" --silence-symbol="<eps>" \
    $bad_utts_dir/analysis/per_utt_details.txt $bad_utts_dir/lattice_oracle/score_$lmwt/${train_set}.ctm \
    $bad_utts_dir/lattice_oracle/score_$lmwt/${train_set}.ctm.edits

  mkdir -p $bad_utts_dir/cleanup

  python utils/ctm_eval_to_goodness.py \
    --word-list=<(cat data/lang/words.txt | awk '{print $1}') \
    --oov-symbol="<unk>" \
    $bad_utts_dir/lattice_oracle/score_$lmwt/train.ctm.edits \
    $bad_utts_dir/cleanup/segments $bad_utts_dir/cleanup/text
  
  steps/cleanup/resegment_data_dir.sh data/$mic/$train_set \
    $bad_utts_dir/cleanup/segments $bad_utts_dir/cleanup/text \
    data/$mic/${train_set}_filtered
  utils/fix_data_dir.sh data/$mic/${train_set}_filtered

  if [ $latgen_train_set != $train_set ]; then
    steps/cleanup/resegment_data_dir.sh data/$mic/${latgen_train_set} \
      $bad_utts_dir/cleanup/segments $bad_utts_dir/cleanup/text \
      data/$mic/${train_set}_filtered
    utils/fix_data_dir.sh data/$mic/${latgen_train_set}_filtered
  fi
  
  utils/copy_data_dir.sh data/$mic/${train_set}_filtered data/$mic/${train_set}_filtered_hires
  steps/make_mfcc.sh --mfcc-config conf/mfcc_hires.conf --nj 100 \
    data/$mic/${train_set}_filtered_hires exp/$mic/make_hires mfcc_${mic}_hires
  steps/compute_cmvn_stats.sh data/$mic/${train_set}_filtered_hires exp/$mic/make_hires mfcc_${mic}_hires
  utils/fix_data_dir.sh data/$mic/${train_set}_filtered_hires
fi
