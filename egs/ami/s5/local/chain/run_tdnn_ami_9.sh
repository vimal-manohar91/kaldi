...
if [ ! -z "$pad_length_left" ] || [ ! -z "$pad_length_right" ]; then
  if [ $stage -le 12 ]; then
    tmpdir=exp/pad_segments_${train_set}

    nj=100

    utils/copy_data_dir.sh data/$mic/$train_set data/$mic/${train_set}_pad${pad_length_left}_${pad_length_right}

    utils/split_data.sh data/$mic/$train_set $nj

    sdata=data/$mic/$train_set/split$nj

    $train_cmd JOB=1:$nj $tmpdir/log/get_reco_lengths.JOB.log \
      wav-to-duration scp:$sdata/JOB/wav.scp ark,t:$tmpdir/reco_lengths.JOB.ark

    for n in `seq $nj`; do
      cat $tmpdir/reco_lengths.$n.ark
    done > $tmpdir/reco_lengths.ark

    steps/cleanup/pad_segments.py --reco-lengths=$tmpdir/reco_lengths.ark \
      --pad-length-left=$pad_length_left --pad-length-right=$pad_length_right \
      data/$mic/$train_set/segments \
      data/$mic/${train_set}_pad${pad_length_left}_${pad_length_right}/segments

    rm data/$mic/${train_set}_pad${pad_length_left}_${pad_length_right}/{feats.scp,cmvn.scp}
  fi
...
if [ $stage -le 13 ]; then
  ...
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d mfcc/storage ]; then
    utils/create_split_dir.pl \
     /export/b{01,02,12,13}/$USER/kaldi-data/egs/ami-$(date +'%m_%d_%H_%M')/s5/mfcc/storage mfcc/storage
  fi

  steps/make_mfcc.sh --mfcc-config conf/mfcc.conf --cmd "$train_cmd" --nj 100 \
    data/$mic/${train_set} exp/$mic/make_mfcc mfcc
  steps/compute_cmvn_stats.sh \
    data/$mic/${train_set} exp/$mic/make_mfcc mfcc
  utils/fix_data_dir.sh data/$mic/$train_set

  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d mfcc_hires/storage ]; then
    utils/create_split_dir.pl \
     /export/b{01,02,12,13}/$USER/kaldi-data/egs/ami-$(date +'%m_%d_%H_%M')/s5/mfcc_hires/storage mfcc_hires/storage
  fi

  utils/copy_data_dir.sh data/$mic/$train_set data/$mic/${train_set}_hires
  steps/make_mfcc.sh --mfcc-config conf/mfcc_hires.conf --cmd "$train_cmd" --nj 100 \
    data/$mic/${train_set}_hires exp/$mic/make_hires mfcc_hires
  steps/compute_cmvn_stats.sh \
    data/$mic/${train_set}_hires exp/$mic/make_hires mfcc_hires
