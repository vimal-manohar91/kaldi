...
if [ $stage -le -4 ]; then
  ...
  for lang in $(seq 0 $[$num_lang-1]); do
    ...
    $cmd $dir/$lang/log/reinitialize.log \
      nnet-am-reinitialize --remove-fixed-scale-component=true $input_model ${alidir[$lang]}/final.mdl $dir/$lang/0.mdl || exit 1;
  done

  cp `dirname $input_model`/*.config $dir
fi
...
if [ $stage -le -3 ]; then
  ...
  for lang in $(seq 0 $[$num_lang-1]); do
    ...
    if [ "$presoftmax_prior_scale_power" != "0.0" ]; then
      ...
      nj=$(cat ${alidir[$lang]}/num_jobs)
      # obtains raw pdf count    
      $cmd JOB=1:$nj $dir/$lang/log/acc_pdf.JOB.log \
        ali-to-post "ark:gunzip -c ${alidir[$lang]}/ali.JOB.gz|" ark:- \| \
        post-to-tacc --per-pdf=true --binary=false ${alidir[$lang]}/final.mdl ark:- $dir/$lang/JOB.pacc || exit 1;
      cat $dir/$lang/*.pacc > $dir/$lang/pacc
      rm $dir/$lang/*.pacc
      ...
      for (i=2; i<=NF-1; i++) {total+=sum[i]} 
        ...
        }' $dir/$lang/pacc > $dir/$lang/presoftmax_prior_scale_vecfile

        echo "FixedScaleComponent scales=$dir/$lang/presoftmax_prior_scale_vecfile" > $dir/$lang/per_element.config
        echo "insert an additional layer of FixedScaleComponent before softmax"
        inp=`nnet-am-info $dir/$lang/0.mdl | grep 'Softmax' | awk '{print $2}'`
        nnet-init $dir/$lang/per_element.config - | nnet-insert --insert-at=$inp --randomize-next-component=false $dir/$lang/0.mdl - $dir/$lang/0.mdl
    fi
  done
fi
...
# do any mixing-up after quarter the iters.
mix_up_iter=$[$num_iters/4]
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    if [ $x -gt 0 ] && \
      ...
      srcdir=`dirname $input_model`
      if [ ! -f $srcdir/hidden_${cur_num_hidden_layers}.config ]; then
        echo "Could not find $srcdir/hidden_${cur_num_hidden_layers}.config file!" && exit 1
      fi
      cp $srcdir/hidden_${cur_num_hidden_layers}.config $dir
    else
      do_average=true
    ...
    ( # this sub-shell is so that when we "wait" below,
      ...
      for lang in $(seq 0 $[$num_lang-1]); do
        ...
        if [ $x -gt 0 ] && \
          ...
          [ $[$x%$add_layers_period] -eq 0 ]; then
          cur_num_hidden_layers=$[$x/$add_layers_period];
          inp=`nnet-am-info $dir/$lang/$x.mdl | grep 'Softmax' | awk '{print $2}'`
          ...
          echo "Current number of hidden layers for lang $lang : ${cur_num_hidden_layers}. Adding new hidden layer now."
          mdl="nnet-init --srand=$x $dir/hidden_${cur_num_hidden_layers}.config - | nnet-insert --insert-at=$inp $dir/$lang/$x.mdl - - |"
        else
          mdl=$dir/$lang/$x.mdl
        fi
        ...
        for n in $(seq $this_num_jobs_nnet); do
          ...
          $cmd $parallel_opts $dir/$lang/log/train.$x.$n.log \
            ...
            "ark:nnet-copy-egs --frame=$frame ark:${egs_dir[$lang]}/egs.$archive.ark ark:- | nnet-shuffle-egs --buffer-size=$shuffle_buffer_size --srand=$x ark:- ark:-|" \
    ...
    (
      ...
      for lang in $(seq 0 $[$num_lang-1]); do
        if $do_averate; then
          ...
          cp $dir/$lang/$[$x+1].$n.mdl $dir/$lang/$[$x+1].mdl || exit 1;
        fi
      done
      wait
