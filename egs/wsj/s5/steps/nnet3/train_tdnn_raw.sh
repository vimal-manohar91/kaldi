...
case $feat_type in
  raw) feat_dim=`compute-raw-frame-feats --config=$raw_conf "scp:head -n 1 $data/wav.scp |" ark,t:- | head -n 2 | tail -n 1 | wc -w`
...
echo splice_indexes = $splice_indexes 
if [ $stage -le -5 ]; then
...
if [ $stage -le -4 ] && [ -z "$egs_dir" ]; then
  ...
  extra_opts+=(--low-rms $low_rms --high-rms $high_rms)
  echo "$0: calling get_raw_egs.sh"
  steps/nnet3/get_raw_egs.sh $egs_opts "${extra_opts[@]}" --raw-conf $raw_conf \
      --wav-input $wav_input \
