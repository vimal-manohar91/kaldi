...
def get_args():
    ...
    parser.add_argument("--compute-average-posteriors", type=str,
                        action=common_lib.StrToBoolAction,
                        default=False, choices=["true", "false"],
                        help="Compute average posteriors for the purporse "
                        "of using as a prior")
...
def train(args, run_opts, background_process_handler):
    ...
    try:
        ...
        logger.warning("KeyError {0}: Variables need to be defined in "
                       "{1}".format(str(e), '{0}/configs'.format(args.dir)))
