...
# This version of the script, nnet3/chain/train_tdnn_multiple_targets.sh, is for joint 'cross-entropy' and 'chain' training systems.
...
obj_scales="1.0:1.0" # comma-seprated objective scales used to scale different output-node's objective 
                  # and derivatives during training. The order is the same as output-node order in the config.
...
if [ $# != 5 ]; then
  echo "Usage: $0 [opts] <data> <tree-dir> <ali-dir> <phone-lattice-dir> <exp-dir>"
  echo " e.g.: $0 data/train exp/chain/tri3b_tree exp/tri3_ali exp/tri3_latali exp/chain/tdnn_a"
  echo ""
...
if [ $stage -le -4 ] && [ -z "$egs_dir" ]; then
  ...
  echo "$0: calling get_egs_multiple_outputs.sh"
  steps/nnet3/chain/get_egs_multiple_targets.sh $egs_opts "${extra_opts[@]}" \
    ...
    ( # this sub-shell is so that when we "wait" below,
      ...
      for n in $(seq $this_num_jobs); do
        ...
        $cmd $train_queue_opt $dir/log/train.$x.$n.log \
          nnet3-chain-train --apply-deriv-weights=$apply_deriv_weights --objective-scales=$obj_scales \
