...
if [ $# != 5 ]; then
  echo "Usage: $0 [opts] <data> <chain-dir> <ali-dir> <lattice-dir> <egs-dir>"
  echo " e.g.: $0 data/train exp/tri4_nnet exp/tri3_ali exp/tri3_lats exp/tri4_nnet/egs"
  echo ""
...
if [ $stage -le 3 ]; then
  ...
  $cmd $dir/log/create_valid_subset.log \
    ...
    chain-get-supervision $ctc_supervision_all_opts $chaindir/tree $chaindir/0.trans_mdl \
      ark:- ark:- \| \
    nnet3-chain-get-egs-multiple-targets $valid_ivector_opt $valid_egs_opts $chaindir/normalization.fst \
      "$valid_feats" "ark,s,cs:ali-to-pdf $alidir/final.mdl scp:$dir/ali_special.scp ark:- | ali-to-post ark:- ark:- |" \
      ark,s,cs:- "ark:$dir/valid_all.cegs" || touch $dir/.error &
  $cmd $dir/log/create_train_subset.log \
    lattice-align-phones --replace-output-symbols=true $latdir/final.mdl scp:$dir/lat_special.scp ark:- \| \
    chain-get-supervision $ctc_supervision_all_opts \
     $chaindir/tree $chaindir/0.trans_mdl ark:- ark:- \| \
    nnet3-chain-get-egs-multiple-targets $train_subset_ivector_opt $valid_egs_opts $chaindir/normalization.fst \
       "$train_subset_feats" "ark,s,cs:ali-to-pdf $alidir/final.mdl scp:$dir/ali_special.scp ark:- | ali-to-post ark:- ark:- |" \
       ark,s,cs:- "ark:$dir/train_subset_all.cegs" || touch $dir/.error &
  wait;
...
if [ $stage -le 4 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/get_egs.JOB.log \
    ...
    chain-get-supervision $ctc_supervision_all_opts \
      $chaindir/tree $chaindir/0.trans_mdl ark:- ark:- \| \
    nnet3-chain-get-egs-multiple-targets $ivector_opt $egs_opts \
     "$feats" "ark,s,cs:filter_scp.pl $sdata/JOB/utt2spk $dir/ali.scp | ali-to-pdf $alidir/final.mdl scp:- ark:- | ali-to-post ark:- ark:- |" ark,s,cs:- ark:- \| \
