...
if $determinize; then
  lattice_determinize_cmd="lattice-determinize-phone-pruned-parallel --write-compact=false --num-threads=$num_threads --acoustic-scale=$acwt --max-mem=$max_mem --minimize=$minimize --prune=true --beam=$lattice_beam ark:- ark:- |"
fi
