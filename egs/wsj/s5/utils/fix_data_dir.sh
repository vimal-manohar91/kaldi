...
for x in utt2spk spk2utt feats.scp text segments wav.scp cmvn.scp vad.scp \
  reco2file_and_channel spk2gender utt2lang utt2uniq utt2dur $utt_extra_files $spk_extra_files; do
  if [ -f $data/$x ]; then
...
function filter_utts {
  ...
  for x in utt2spk utt2uniq feats.scp vad.scp text segments utt2lang utt2dur $maybe_wav $utt_extra_files; do
    if [ -f $data/$x ]; then
      cp $data/$x $data/.backup/$x
