...
if [ $stage -le 1 ]; then
  local/apollo_flight_journal/extend_lexicon.sh \
    --orig-dict-dir data/local/dict \
    --train-text data/train/text \
    --afj-transcripts data/local/afj/afj_transcripts.txt \
    --dir data/local/dict_afj --stage 4
  utils/validate_dict_dir.pl data/local/dict_afj
  utils/prepare_lang.sh data/local/dict_afj '<unk>' data/local/lang data/lang
fi
...
if [ $stage -le 2 ]; then
  ...
  local/apollo_flight_journal/train_lms.sh \
