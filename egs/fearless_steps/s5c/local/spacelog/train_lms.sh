...
spacelog_transcripts=data/local/spacelog/spacelog_transcripts.txt
dev_text=data/dev/text
dir=data/local/pocolm_spacelog
lang=data/lang
stage=-1
...
if [ $stage -le 0 ]; then
  ...
  cat $spacelog_transcripts | \
    utils/sym2int.pl --map-oov $oov $lang/words.txt | \
    utils/int2sym.pl $lang/words.txt | \
    gzip -c > ${dir}/data/text/spacelog.txt.gz || exit 1;
