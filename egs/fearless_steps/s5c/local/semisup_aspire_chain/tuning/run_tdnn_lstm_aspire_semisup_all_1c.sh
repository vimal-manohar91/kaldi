...
if [ $stage -le 16 ]; then
  ...
  output-layer name=output-1 input=lstm5 output-delay=$label_delay include-log-softmax=false dim=$num_targets max-change=1.5 $output_opts
  ...
  output-layer name=output-1-xent input=lstm5 output-delay=$label_delay dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=1.5 $output_opts
...
if [ $stage -le 22 ]; then
  ...
  nnet3-am-copy --edits="remove-output-nodes name=output;rename-node old-name=output-1 new-name=output" \
    $dir/final.mdl $dir/final-output-1.mdl
  for data in dev; do
    (
