...
stage=0
unsup_data_dir=data/apollo11_whole_legal_1a_seg_300k
...
if [ $stage -le 15 ]; then
  ...
  steps/nnet3/chain/make_weighted_den_fst.sh --cmd "$train_cmd" \
    --lm-opts '--num-extra-lm-states=2000' \
    $best_paths_dir $dir/unsup_den_fst || exit 1
fi
...
if [ $stage -le 16 ]; then
  ...
  tdnn_opts="l2-regularize=0.004 dropout-proportion=0.0 dropout-per-dim-continuous=true"
  tdnnf_opts="l2-regularize=0.004 dropout-proportion=0.0 bypass-scale=0.66"
  linear_opts="l2-regularize=0.004 orthonormal-constraint=-1.0"
