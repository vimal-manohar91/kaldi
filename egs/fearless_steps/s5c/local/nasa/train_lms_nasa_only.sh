...
extra_transcripts="data/local/nasa/spacelog_transcripts.txt data/local/nasa/apollo_html_reports.txt data/local/nasa/a11_html_reports.txt data/local/nasa/alsj_transcripts.txt"
dev_text=data/dev/text
dir=data/local/pocolm_nasa_only
lang=data/lang
stage=-1

bypass_metaparam_optim_opt=
num_ngrams_large=2500000
num_ngrams_small=750000
...
if [ $stage -le 0 ]; then
  ...
  cut -d ' ' -f 2- $train_text | \
    ...
    gzip -c > ${dir}/data/text/dev.txt.gz

  cat $afj_transcripts | \
    utils/sym2int.pl --map-oov $oov $lang/words.txt | \
    utils/int2sym.pl $lang/words.txt | \
    gzip -c > ${dir}/data/text/afj_transcripts.txt.gz || exit 1;
...
#min_counts='train=1 fisher=1 spacelog=1'
min_counts=
...
if [ $stage -le 1 ]; then
  ...
  train_lm.py  --wordlist=${wordlist} --num-splits=10 --warm-start-ratio=20  \
               ...
               --fold-dev-into=afj_transcripts ${bypass_metaparam_optim_opt} \
