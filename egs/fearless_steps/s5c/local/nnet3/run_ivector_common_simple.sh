...
if [ $stage -le 1 ]; then
  ...
  steps/make_mfcc.sh --cmd "$cmd" --nj 17 data/${train_set}_sp || exit 1;
...
if [ $stage -le 2 ]; then
  ...
  steps/align_fmllr.sh --nj 16 --cmd "$cmd" \
...
if [ $stage -le 3 ]; then
  ...
  for datadir in ${train_set}_sp ${test_sets}; do
    steps/make_mfcc.sh --nj 16 --mfcc-config conf/mfcc_hires.conf \
     --cmd "$cmd" data/${datadir}_hires || exit 1;
...
if [ $stage -le 5 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$cmd" --nj 16  \
    --num-frames 700000 \
    --num-threads 4\
    data/${train_set}_sp_hires 512 exp/nnet3${nnet3_affix}/pca_transform \
    exp/nnet3${nnet3_affix}/diag_ubm || exit 1
fi
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$cmd" \
...
if [ $stage -le 7 ]; then
  ...
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$cmd" --nj 16 \
  ...
  for data in $test_sets; do
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$cmd" --nj 6 \
      data/${data}_hires exp/nnet3${nnet3_affix}/extractor exp/nnet3${nnet3_affix}/ivectors_${data}_hires
  done
fi
