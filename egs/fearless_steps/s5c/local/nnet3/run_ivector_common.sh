...
gmm_dir=exp/${gmm}
sgmm_dir=exp/${sgmm}
ali_gmm_dir=exp/${gmm}_ali_${train_set}_sp
...
if [ $stage -le 1 ]; then
  ...
  steps/make_mfcc.sh --cmd "$cmd" --nj $nj data/${train_set}_sp || exit 1;
...
if [ $stage -le 2 ]; then
  ...
  #  data/${train_set}_sp data/lang $gmm_dir $ali_gmm_dir || exit 1
  steps/align_sgmm2.sh --nj $nj --cmd "$cmd" \
    --transform-dir $ali_gmm_dir  \
    data/${train_set}_sp data/lang $sgmm_dir $ali_dir
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$cmd" --nj 30 \
    --num-frames 700000 \
    --num-threads 8 \
