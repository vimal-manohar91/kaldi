...
ali_dir=exp/${sgmm}_ali_${train_set}_sp
...
if [ $stage -le 8 ]; then
  ...
  relu-renorm-layer name=tdnn1 input=Append(-1,0,1) dim=196
  relu-renorm-layer name=tdnn2 input=Append(-1,0,1) dim=196
  relu-renorm-layer name=tdnn3 input=Append(-1,0,1) dim=196
  relu-renorm-layer name=tdnn4 input=Append(-1,0,1) dim=196
  relu-renorm-layer name=tdnn5 input=Append(-1,0,1) dim=196
  relu-renorm-layer name=tdnn6 dim=196
  relu-renorm-layer name=tdnn7 dim=196
  relu-renorm-layer name=tdnn8 dim=196

  output-layer name=output input=tdnn8 dim=$num_targets max-change=1.5
EOF
...
if [ $stage -le 10 ]; then
  ...
  # online decoding.
  graph_dir=exp/sgmm2/graph_pp/
  # use already-built graphs.
  steps/nnet3/decode.sh --nj 256 --cmd "$cmd --max-jobs-run 32" \
      --iter final \
...
#    ${dir}/decode_dev ${dir}/decode_dev.rescored
  steps/lmrescore_const_arpa.sh --cmd "$cmd"  \
    data/lang_test/ data/lang_pp_test_fg/ data/dev \
    $dir/decode_dev $dir/decode_dev.rescored
fi
