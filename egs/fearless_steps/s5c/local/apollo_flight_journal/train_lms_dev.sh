...
if [ $stage -le 0 ]; then
  ...
  cat $afj_transcripts | \
    ...
    gzip -c > ${dir}/data/text/train.txt.gz || exit 1;
  ...
  cat $dev_text | cut -d " " -f 2- > ${dir}/data/text/dev.txt
...
if [ $stage -le 1 ]; then
  ...
  get_data_prob.py ${dir}/data/text/dev.txt ${unpruned_lm_dir} 2>&1 | grep -F '[perplexity' | tee ${unpruned_lm_dir}/perplexity_dev_set.log
fi
  get_data_prob.py ${dir}/data/text/dev.txt ${unpruned_lm_dir} 2>&1 | grep -F '[perplexity' | tee ${unpruned_lm_dir}/perplexity_dev_set.log
...
if [ $stage -le 2 ]; then
  ...
  get_data_prob.py ${dir}/data/text/dev.txt ${dir}/data/lm_${order}_prune_big 2>&1 | grep -F '[perplexity' | tee ${dir}/data/lm_${order}_prune_big/perplexity_dev_set.log
...
if [ $stage -le 3 ]; then
  ...
  get_data_prob.py ${dir}/data/text/dev.txt ${dir}/data/lm_${order}_prune_small 2>&1 | grep -F '[perplexity' | tee ${dir}/data/lm_${order}_prune_small/perplexity_dev_set.log
