...
afj_output=/export/fs01/vmanoha1/fearless_steps/apollo_journal/afj_transcript_lines_v1
dir=data/local/afj
...
if [ $stage -le 2 ]; then
  local/apollo_flight_journal/parse_afj_transcripts.py \
    $afj_output/apollo11_transcripts.jl /dev/stdout | \
    local/dict/normalize_abbrv.py $dict/lexicon.txt \
    > $dir/afj_a11_transcripts.txt
fi
