...
issrilm=`which ngram-count`
if [ -z $issrilm ]; then
  echo "ngram-count was not found. You must install the SRILM package."
  exit 1;
fi
...
if [ $stage -le 1 ]; then
  ...
  phonetisaurus-align $align_opts --input=$outdir/lexicon_tab_separated.txt --ofile=${outdir}/aligned_lexicon.corpus || exit 1;
fi

if [ $stage -le 2 ]; then
  # Convert aligned lexicon to arpa using srilm.
  ngram-count $srilm_opts\
