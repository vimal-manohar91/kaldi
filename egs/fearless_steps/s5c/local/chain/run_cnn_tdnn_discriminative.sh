...
srcdir=exp/chain_cleaned/cnn_tdnn1a_sp
train_data_dir=data/train_seg_cleaned_sp_hires
online_ivector_dir=exp/nnet3${nnet3_affix}/ivectors_train_seg_cleaned_sp_hires
...
if [ $frame_subsampling_factor -ne 1 ]; then
  if [ $stage -le 0 ]; then
    ...
    for x in `seq -$[frame_subsampling_factor/2] $[frame_subsampling_factor/2]`; do
      steps/shift_feats.sh --cmd "$cmd --max-jobs-run 40" --nj 30 \
...
if [ $stage -le 1 ]; then
  ...
  nj=342 # have a high number of jobs because this could take a while, and we might
...
if [ -z "$lats_dir" ]; then
  ...
  if [ $stage -le 2 ]; then
    ...
    steps/nnet3/make_denlats.sh --cmd "$cmd" \
  ...
  if [ $stage -le 3 ]; then
    if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d ${srcdir}_degs/storage ]; then
      utils/create_split_dir.pl \
        /export/b0{3,6,7,8}/$USER/kaldi-data/egs/swbd-$(date +'%m_%d_%H_%M')/s5/${srcdir}_degs/storage ${srcdir}_degs/storage
    fi
    ...
    steps/nnet3/get_egs_discriminative.sh \
      --cmd "$cmd --max-jobs-run $max_jobs --mem 20G" --stage $get_egs_stage --cmvn-opts "$cmvn_opts" \
  ...
  for data in $test_sets; do
    (
      ...
      steps/nnet3/decode.sh \
        ...
        $tree_dir/graph data/${data}_hires ${dir}/decode_${data}_${iter} || exit 1
     )  || touch $dir/.error &
    done
  done
  wait
