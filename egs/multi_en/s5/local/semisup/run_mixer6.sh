...
set -o pipefail
exp_root=exp/multi_a
supervised_data=data/multi_a/tri5b
stage=0

. utils/parse_options.sh

if [ $stage -le 0 ]; then
  local/mixer6_calls_prepare_data.py /export/LDC/LDC2013S03/mx6_speech data/local/mixer6
  local/mixer6_format_data.sh data/local/mixer6 data/train_mixer6
fi
...
if [ $stage -le 2 ]; then
 steps/segmentation/detect_speech_activity.sh --stage $sad_stage \
   --cmd "$train_cmd" --nj 400 --convert-data-dir-to-whole false \
   --extra-left-context 79 --extra-right-context 21 \
   --extra-left-context-initial 0 --extra-right-context-final 0 \
   --frames-per-chunk 150 --mfcc-config sad_model/conf/mfcc_hires.conf \
   data/train_mixer6 sad_model/exp/segmentation_1a/tdnn_stats_asr_sad_1a \
   sad_model/mfcc_hires sad_model/exp/segmentation_1a/tdnn_stats_asr_sad_1a \
   data/train_mixer6_1a
fi

for f in data/train_mixer6_1a_seg/utt2spk \
  data/train_mixer6_1a_seg/feats.scp; do
  if [ ! -f $f ]; then
