...
if [ $stage -le 0 ]; then
  ...
  local/make_swbd2_phase1.pl /export/LDC/LDC98S75 \
    data/swbd2_phase1_train
  local/make_swbd2_phase2.pl /export/LDC/LDC99S79 \
    data/swbd2_phase2_train
  local/make_swbd2_phase3.pl /export/LDC/LDC2002S06 \
    data/swbd2_phase3_train
  local/make_swbd_cellular1.pl /export/LDC/LDC2001S13 \
    data/swbd_cellular1_train
  local/make_swbd_cellular2.pl /export/LDC/LDC2004S07 \
...
if [ $stage -le 1 ]; then
  ...
  local/make_sre.pl /export/LDC/LDC2006S44 04 \
    data/local/sre/speaker_list data/sre2004
fi

if [ $stage -le 2 ]; then
  local/make_mx6_calls.pl /export/LDC/LDC2013S03 data/local/mx6

  for mic in 02 04 05 06 07 08 09 10 11 12 13; do
    local/make_mx6_mic.pl /export/LDC/LDC2013S03 $mic data/local/mx6
  done
