...
decode_dir_root=exp/multi_a/chain/tdnn_lstm_1c2_sp_ld5/decode_multi_a_tri5a_fsh_sw1_tg
lang_dir=data/lang_multi_a_tri5a_fsh_sw1_fg
...
librispeech_lm_corpus=librispeech-lm-norm.txt.gz
...
if [ $stage -le 0 ]; then
  ...
  cat $fisher_text | cut -d " " -f2- >   $text_dir/fisher.txt
  cat $wsj_text | cut -d " " -f2- >   $text_dir/wsj.txt
  cat $tedlium_text | cut -d " " -f2- >   $text_dir/tedlium.txt
  cat $hub4_en_text | cut -d " " -f2- >   $text_dir/hub4_en.txt
  gunzip -c $librispeech_lm_corpus | tr 'A-Z' 'a-z' > $text_dir/librispeech.txt
fi

if [ $stage -le 1 ]; then
  cp $lang_dir/words.txt $dir/config/
...
if [ $stage -le 3 ]; then
  rnnlm/train_rnnlm.sh --num-jobs-initial 1 --num-jobs-final 3 \
                  --stage $train_stage --num-epochs 10 --cmd "$train_cmd --mem 4G" \
                  --num-egs-threads 4 --compute-prob-opt "--mem 8G" $dir 
fi
...
#  LM=sw1_tg # if using the original 3-gram G.fst as old lm
  ...
  for decode_set in eval2000 rt03; do
    decode_dir=${decode_dir_root}_${decode_set}${decode_affix}
    # Lattice rescoring
