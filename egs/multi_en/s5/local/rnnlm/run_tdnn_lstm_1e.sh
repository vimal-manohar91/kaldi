...
ac_model_dir=exp/multi_a/chain/tdnn_6a_sp
...
swbd_text=data/swbd/train/text
fisher_text=data/fisher/train/text
wsj_text=data/wsj/train/text
tedlium_text=data/tedlium/train/text
librispeech_text=data/librispeech/train/text
librispeech_text=librispeech.txt
rt03_text=data/rt03_hires/text
hub4_en_text=data/hub4_en/train/text
...
cat data/librispeech_*/train/text > librispeech.txt

if [ $stage -le 0 ]; then
  mkdir -p $text_dir
  echo -n >$text_dir/dev.txt
  # hold out one in every 50 lines as dev data.
  cat $swbd_text | cut -d ' ' -f2- | awk -v text_dir=$text_dir '{if(NR%50 == 0) { print >text_dir"/dev.txt"; } else {print;}}' >$text_dir/swbd.txt
  cat $PWD/$fisher_text      | cut -d " " -f2- >   $text_dir/fisher.txt
  cat $PWD/$wsj_text         | cut -d " " -f2- >   $text_dir/wsj.txt
  cat $PWD/$tedlium_text     | cut -d " " -f2- >   $text_dir/tedlium.txt
  cat $PWD/$librispeech_text | cut -d " " -f2- >   $text_dir/librispeech.txt
  cat $PWD/$rt03_text        | cut -d " " -f2- >   $text_dir/rt03.txt
  cat $PWD/$hub4_en_text     | cut -d " " -f2- >   $text_dir/hub4_en.txt
fi

if [ $stage -le 1 ]; then
  cp data/lang_multi_a_tri6a/words.txt $dir/config/
...
# lang_multi_a_tri6a_fsh_sw1_tg
# decode_eval2000_fsh_sw1_tg

LM=multi_a_tri6a_fsh_sw1_tg # using the 4-gram const arpa file as old lm
...
#  LM=sw1_tg # if using the original 3-gram G.fst as old lm
  ...
  for decode_set in eval2000; do
    decode_dir=${ac_model_dir}/decode_${decode_set}_fsh_sw1_tg
