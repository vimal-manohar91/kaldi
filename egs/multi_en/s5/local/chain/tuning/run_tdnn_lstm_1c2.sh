...
if $test_online_decoding && [ $stage -le 16 ]; then
  ...
  steps/online/nnet3/prepare_online_decoding.sh \
       ...
       $lang exp/$multi/nnet3${nnet3_affix}/extractor $dir ${dir}_online
  ...
  for decode_set in eval2000 rt03; do
    (
      ...
      steps/online/nnet3/decode.sh --nj $decode_nj --cmd "$decode_cmd" $iter_opts \
          ...
          --extra-left-context-initial 0 \
         $graph_dir data/${decode_set}_hires \
         ${dir}_online/decode${lang_suffix}_${decode_set}${decode_dir_affix:+_$decode_dir_affix}${decode_iter:+_$decode_iter} || exit 1;
        
      steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" \
        $lang_dir $rescore_lang_dir data/${decode_set}_hires \
        ${dir}_online/decode${lang_suffix}_${decode_set}${decode_dir_affix:+_$decode_dir_affix}{,_fg}${decode_iter:+_iter$decode_iter} || exit 1;
      ) || touch $dir/.error &
  done
  wait
  if [ -f $dir/.error ]; then
