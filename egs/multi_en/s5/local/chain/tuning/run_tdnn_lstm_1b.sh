...
treedir=exp/$multi/chain${chain_affix}/${gmm}_tree
...
if [ $stage -le 9 ]; then
  steps/align_fmllr_lats.sh --nj 100 --cmd "$train_cmd" \
    --generate-ali-from-lats data/$train_set \
...
if [ $stage -le 12 ]; then
  ...
  relu-batchnorm-layer name=tdnn8 input=Append(-3,0,3) dim=$relu_dim
  relu-batchnorm-layer name=tdnn9 input=Append(-3,0,3) dim=$relu_dim
...
if [ $stage -le 15 ]; then
  ...
  for decode_set in eval2000; do
      ...
      steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" \
          $lang_dir $rescore_lang_dir data/${decode_set}_hires/test \
