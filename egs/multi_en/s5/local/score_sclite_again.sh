...
# The source commit was 5dfa20aa3da217cc4d51d2e844995db1139b7bcd
...
# Remove some stuff we don't want to score, from the ctm.
  for y in $dir/score_*/$name.ctm.bk; do
    x=$(echo $y | perl -pe 's/\.bk$//g') || exit 1
    if [ -f $x.bk ]; then
      cp $x.bk $x || exit 1
    fi
    cp $x $dir/tmpf;
    cat $dir/tmpf | grep -i -v -E '\[NOISE|LAUGHTER|VOCALIZED-NOISE\]' | \
      grep -i -v -E '<UNK>' > $x || exit 1
#      grep -i -v -E '<UNK>|%HESITATION' > $x;  # hesitation is scored
    case "$name" in eval2000* )
      python local/format_acronyms_ctm_eval2000.py -i $x -o $x.mapped || exit 1
     ;;
    rt03* )
      python local/format_acronyms_ctm_rt03.py -i $x -o $x.mapped || exit 1
     ;;
    esac
    cp $x $x.bk
    mv $x.mapped $x
