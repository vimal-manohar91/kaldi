...
pruned_rescore=true
undeterminized_lattices=true

text=data/local/pocolm/text_all.gz
lexicon=data/local/dict/lexiconp.txt

decode_set=train_unsup100k_250k_n10k_sp
lm_suffix=poco_ex250k_unk
decode_sets=eval2000
rescore_opts=

. ./cmd.sh
. ./utils/parse_options.sh

text_dir=data/rnnlm/text${affix}
...
mhm um-hum
EOF
  gunzip -c $text | cut -d ' ' -f2- | awk -v text_dir=$text_dir '{if(NR%50 == 0) { print >text_dir"/dev.txt"; } else {print;}}' | \
    awk 'NR==FNR{a[$1]=$2;next}{for (n=1;n<=NF;n++) if ($n in a) $n=a[$n];print $0}' \
    $dir/config/hesitation_mapping.txt - > $text_dir/train.txt
fi
...
#  LM=sw1_tg # if using the original 3-gram G.fst as old lm
  ...
  if $undeterminized_lattices; then
    undet=_undeterminized
  fi
  for decode_set in $decode_sets; do
    decode_dir=${ac_model_dir}/decode_${decode_set}${lm_suffix:+_$lm_suffix}

    # Lattice rescoring
    rnnlm/lmrescore${pruned}${undet}.sh \
      --cmd "$decode_cmd --mem 4G" ${rescore_opts} \
      --weight 0.45 --max-ngram-order $ngram_order --skip-scoring true \
      data/lang_test${lm_suffix:+_$lm_suffix} $dir \
