...
if [ $stage -le 9 ]; then
  local/semisup/chain/run_tdnnf_1a.sh \
    --train-set train_sup \
    --ivector-train-set "" \
...
if [ $stage -le 10 ]; then
  local/semisup/chain/run_tdnnf_100k_semisupervised_1a.sh \
    --supervised-set train_sup \
    --unsupervised-set train_unsup100k_250k \
    --sup-chain-dir $exp_root/chain/tdnnf_1a_sp \
...
if [ $stage -le 11 ]; then
  local/semisup/chain/tuning/run_tdnnf_1a.sh \
