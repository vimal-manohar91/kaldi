...
tdnn_affix=7d_h1024  # affix for the supervised chain-model directory
...
mmi_factor_schedule="output-0=1.0,1.0 output-1=0.2,0.2"
smbr_factor_schedule="output-0=0.0,0.0 output-1=0.4,0.4"

# Semi-supervised options
comb_affix=comb_500k_1e_smbr  # affix for new chain-model directory trained on the combined supervised+unsupervised subsets
...
if false && [ $stage -le 1 ]; then
  ...
  local/semisup/chain/run_tdnn_lstm_100k.sh $train_supervised_opts --remove-egs false \
...
chaindir=$exp/chain${nnet3_affix}/tdnn_lstm${tdnn_affix}_sp
...
unsup_lat_dir=$chaindir/decode_${unsupervised_set}_sp${decode_affix}_post_pruned
...
for dset in $unsupervised_set; do
  ...
  if [ $stage -le 5 ]; then
    ...
    steps/nnet3/decode_semisup.sh --num-threads 4 --nj $decode_nj --cmd "$decode_cmd" \
              ...
              --extra-right-context $extra_right_context \ --extra-left-context-initial 0 --extra-right-context-final 0 \
  ...
  if [ $stage -le 6 ]; then
    steps/posterior_prune_lattices.sh --cmd "$decode_cmd --mem 4G" \
      --min-prob $min_prob --min-relative-prob $min_relative_prob --beam 8.0 \
      --write-compact false --word-determinize false \
      $chaindir/decode_${dset}_sp${decode_affix} \
      $chaindir/decode_${dset}_sp${decode_affix}_post_pruned || exit 1
  fi
done

cp $chaindir/final.mdl $unsup_lat_dir
cp $chaindir/tree $unsup_lat_dir
cp $chaindir/frame_subsampling_factor $unsup_lat_dir || true
...
if [ $stage -le 15 ]; then
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --tree-dir $treedir \
    --lat-dir $sup_lat_dir \
    --dir $dir  --lang data/lang_chain_unk || exit 1;
fi
