...
comb_affix=comb1a_smbr  # affix for new chain-model directory trained on the combined supervised+unsupervised subsets
...
if false && [ $stage -le 1 ]; then
  ...
  local/semisup/chain/run_tdnn_lstm_15k.sh $train_supervised_opts --remove-egs false \
...
sup_lat_dir=$exp/chain${chain_affix}/tri3_${supervised_set}_unk_lats
