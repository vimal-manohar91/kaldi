...
comb_affix=comb_250k_ex250k_1d_smbr  # affix for new chain-model directory trained on the combined supervised+unsupervised subsets
...
for dset in $unsupervised_set; do
  ...
  if [ $stage -le 6 ]; then
    rnnlm/lmrescore_undeterminized.sh --cmd "$decode_cmd" \
      --weight 0.45 --max-ngram-order $ngram_order --skip-scoring true \
      ${unsup_decode_lang} $rnnlm_dir \
      data/${dset}_sp_hires \
      $chaindir/decode_${dset}_sp${decode_affix} \
      $chaindir/decode_${dset}_sp${decode_affix}${rnnlm_affix}
  fi

  ln -sf ../final.mdl $chaindir/decode_${dset}_sp${decode_affix}${rnnlm_affix} || true
done
...
decode_affix=${decode_affix}${rnnlm_affix}
