...
# and 1000 hours unsupervised data with naive splitting.
...
tdnn_affix=_semisup100k_1000k_ex1000k_1a  # affix for semi-supervised chain system

# Datasets -- Expects data/$supervised_set and data/$unsupervised_set to be
# present
supervised_set=train_sup
unsupervised_set=train_unsup100k_1000k
...
lm_weights=4,1  # Weights on phone counts from supervised, unsupervised data for denominator FST creation
...
unsup_decode_lang=data/lang_test_poco_ex1000k_unk
unsup_decode_graph_affix=_poco_ex1000k_unk
...
if [ $stage -le 5 ]; then
  ...
  steps/nnet3/decode_semisup.sh --num-threads 4 --nj $nj --cmd "$decode_cmd" --sub-split $nj \
...
if [ $stage -le 8 ]; then
  steps/best_path_weights.sh --cmd "${train_cmd}" --acwt 0.1 \
    $sup_chain_dir/decode${unsup_decode_graph_affix}_${unsupervised_set_perturbed} \
...
if $use_smart_splitting; then
  ...
  if [ ! -z "$word_graph_prune_beam" ]; then
    get_egs_script=steps/nnet3/chain/get_egs_split_wordG.sh
    unsup_egs_opts="$unsup_egs_opts --word-graph-prune-beam $word_graph_prune_beam"
  fi
else
