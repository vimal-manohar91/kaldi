...
if [ $stage -le 11 ]; then
  ...
  output-layer name=output input=lstm4 output-delay=$label_delay dim=$num_targets include-log-softmax=false learning-rate-factor=$learning_rate_factor max-change=1.5
...
if [ $stage -le 14 ]; then
  steps/nnet3/multilingual/combine_egs.sh --cmd "$train_cmd" \
    ...
    --lang2weight $supervision_weights --lang2num-copies "$num_copies" --egs-prefix cegs. 2 \
