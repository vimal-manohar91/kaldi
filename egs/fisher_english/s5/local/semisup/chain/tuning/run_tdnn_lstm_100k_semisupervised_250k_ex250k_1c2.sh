...
unsup_rescore_lang=exp/rnnlm_tdnn_lstm_train_ex250k
unsup_rescore_suffix=_rnnlm_1a_noncompact
...
if [ $stage -le 6 ]; then
  echo "$0: getting the decoding lattices for the unsupervised subset using the chain model at: $sup_chain_dir/decode${unsup_decode_graph_affix}_${unsupervised_set_perturbed}"
  rnnlm/lmrescore_pruned_undeterminized.sh --write-compact false \
...
cp $sup_chain_dir/final.mdl ${unsup_lat_dir}${unsup_rescore_suffix} 
