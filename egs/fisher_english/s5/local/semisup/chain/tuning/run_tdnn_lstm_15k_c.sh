...
# This is similar to _b, but uses unconstrained egs
...
unsupervised_set=train_unsup100_250k
...
if [ $stage -le 11 ]; then
  ...
  if [ -f $treedir/final.mdl ]; then
    echo "$0: $treedir/final.mdl exists! Remove it or skip this stage."
    exit 1
  fi
...
if [ $stage -le 13 ]; then
  ...
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --egs.opts "--frames-overlap-per-eg 0 --generate-egs-scp true --constrained false" \
