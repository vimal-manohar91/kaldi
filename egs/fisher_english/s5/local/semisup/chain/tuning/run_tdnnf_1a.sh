...
dir=$exp_root/chain${chain_affix}/tdnnf${tdnn_affix}_sp
...
if [ -z "$common_treedir" ]; then
  if [ $stage -le 12 ]; then
    if [ -f $treedir/final.mdl ]; then
      echo "$treedir/final.mdl exists. Refusing to rewrite!"
      exit 1
    fi
...
if [ $stage -le 13 ]; then
  ...
  affine_opts="l2-regularize=$l2_regularize dropout-proportion=0.0 dropout-per-dim=true dropout-per-dim-continuous=true"
  tdnnf_opts="l2-regularize=$l2_regularize dropout-proportion=0.0 bypass-scale=0.66"
  linear_opts="l2-regularize=$l2_regularize orthonormal-constraint=-1.0"
  prefinal_opts="l2-regularize=$l2_regularize"
  output_opts="l2-regularize=$output_l2_regularize"
  ...
  tdnnf-layer name=tdnnf2 $tdnnf_opts dim=$hidden_dim bottleneck-dim=160 time-stride=1
  tdnnf-layer name=tdnnf3 $tdnnf_opts dim=$hidden_dim bottleneck-dim=160 time-stride=1
  tdnnf-layer name=tdnnf4 $tdnnf_opts dim=$hidden_dim bottleneck-dim=160 time-stride=1
  tdnnf-layer name=tdnnf5 $tdnnf_opts dim=$hidden_dim bottleneck-dim=160 time-stride=0
EOF

  for n in $(seq 6 $num_layers); do
    cat <<EOF >> $dir/configs/network.xconfig
  tdnnf-layer name=tdnnf${n} $tdnnf_opts dim=$hidden_dim bottleneck-dim=160 time-stride=3
EOF
  done
  ...
  prefinal-layer name=prefinal-chain input=prefinal-l $prefinal_opts big-dim=$hidden_dim small-dim=256
  output-layer name=output include-log-softmax=false dim=$num_targets $output_opts

  prefinal-layer name=prefinal-xent input=prefinal-l $prefinal_opts big-dim=$hidden_dim small-dim=256
