...
# This script is same as _c, but uses unconstrained egs
...
sup_lat_dir=$exp/chain${chain_affix}/tri3_${supervised_set}_lats
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 13 ]; then
    ...
    $get_egs_script --cmd "$decode_cmd --h-rt 100:00:00" --alignment-subsampling-factor 1 \
               ...
               --online-ivector-dir $exp/nnet3${nnet3_affix}/ivectors_${semisup_train_set}_sp_hires \
               --generate-egs-scp true --constrained $unsup_constrained $unsup_egs_opts \
