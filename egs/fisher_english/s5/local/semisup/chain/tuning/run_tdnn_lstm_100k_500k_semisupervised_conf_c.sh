...
comb_affix=comb_500k_1c  # affix for new chain-model directory trained on the combined supervised+unsupervised subsets
...
if [ $stage -le 11 ]; then
  ...
  output-layer name=output input=lstm3 output-delay=$label_delay dim=$num_targets include-log-softmax=false learning-rate-factor=$learning_rate_factor max-change=1.5
