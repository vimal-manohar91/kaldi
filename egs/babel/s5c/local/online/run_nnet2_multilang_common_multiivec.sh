...
if [ $stage -le 4 ]; then
  data_dirs=
  for i in `seq 0 $nlangs`; do
    data_dirs="$data_dirs $data_multilang/${lang[$i]}_hires"
  done
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
    ...
    exp/nnet2_online/${lang[0]}/tri4b exp/nnet2_online/multilang/diag_ubm
fi

if [ $stage -le 5 ]; then
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    $data_multilang/train_hires exp/nnet2_online/multilang/diag_ubm \
    exp/nnet2_online/multilang/extractor || exit 1
fi
...
if [ $stage -le 6 ]; then
  ...
  for i in `seq 0 $nlangs`; do
    ...
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
      $data_multilang/${lang[i]}_hires_max2 exp/nnet2_online/multilang/extractor \
