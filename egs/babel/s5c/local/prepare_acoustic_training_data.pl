...
GetOptions("fragmentMarkers=s" => \$fragMarkers,
           ...
           "convert-word-to-phone=s" => \$convert_word_to_phone
           );
...
if ($convert_word_to_phone eq "true") {
	$textFileName = "$outDir/text";
	print STDERR "Converting word transcript to phone transcript: dictfile = $vocabFile, transcript = $textFileName\n";
	$cmd="tmp=`mktemp`; cp $textFileName \$tmp; perl utils/transnorm.pl --map-to-col -1  \$tmp $vocabFile > $textFileName ; rm -rf \$tmp;";
	system($cmd);
}
