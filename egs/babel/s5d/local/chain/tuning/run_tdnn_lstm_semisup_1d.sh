...
affix=_semisup_1d  # affix for new chain-model directory trained on the combined supervised+unsupervised subsets
...
lang=data/lang_unk_chain
unsup_decode_lang=data/langp_unk_test
unsup_decode_graph_affix=

test_lang=data/langp_unk_test
test_graph_affix=
...
if [ $stage -le 12 ]; then
  ...
  steps/nnet3/decode_semisup.sh --num-threads 4 --nj $decode_nj --cmd "$decode_cmd" \
            ...
            $graphdir data/${unsupervised_set}_sp_hires $srcdir/decode${unsup_decode_graph_affix}_${unsupervised_set}_sp
fi
...
if [ $stage -le 13 ]; then
  steps/best_path_weights.sh --cmd "${train_cmd}" --acwt 0.1 \
    ...
    $srcdir/decode${unsup_decode_graph_affix}_${unsupervised_set}_sp \
    $srcdir/best_path${unsup_decode_graph_affix}_${unsupervised_set}_sp
  echo $frame_subsampling_factor > $srcdir/best_path${unsup_decode_graph_affix}_${unsupervised_set}_sp/frame_subsampling_factor
fi
...
if [ $stage -le 14 ]; then
  steps/nnet3/chain/make_weighted_den_fst.sh --num-repeats $lm_weights \
    --cmd "$train_cmd" \
    ${treedir} ${srcdir}/best_path${unsup_decode_graph_affix}_${unsupervised_set}_sp \
    $dir
fi
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 17 ]; then
    ...
    $get_egs_script --cmd "$decode_cmd --h-rt 100:00:00" --alignment-subsampling-factor 1 \
               ...
               --deriv-weights-scp $srcdir/best_path${unsup_decode_graph_affix}_${unsupervised_set}/weights.scp \
...
if [ $stage -le 19 ]; then
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --tree-dir $treedir \
    --lat-dir $sup_lat_dir \
    --dir $dir --lang data/lang_unk_chain || exit 1;
fi
