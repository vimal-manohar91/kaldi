...
affix=_semisup_1c  # affix for new chain-model directory trained on the combined supervised+unsupervised subsets
...
if [ $stage -le 14 ]; then
  steps/posterior_prune_lattices.sh --acwt 0.1 --beam 8.0 \
    --cmd "$decode_cmd" \
    $srcdir/decode_${unsupervised_set}_sp \
    $srcdir/decode_${unsupervised_set}_sp_post_pruned
  ln -sf ../final.mdl $srcdir/decode_${unsupervised_set}_sp_post_pruned/ || true
fi
...
unsup_lat_dir=${srcdir}/decode_${unsupervised_set}_post_pruned
