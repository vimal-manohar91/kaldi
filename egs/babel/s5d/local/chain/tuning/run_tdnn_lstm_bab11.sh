...
train_stage=-10
tree_affix=_b  # affix for tree directory, e.g. "a" or "b", in case we change the configuration.
tdnn_affix="_bab11"  #affix for TDNN directory, e.g. "a" or "b", in case we change the configuration.
...
lat_dir=exp/chain${nnet3_affix}/${gmm}_${train_set}_sp_unk_lats
...
if [ $stage -le 14 ]; then
  ...
  if [ -d data/lang_unk_chain ]; then
    if [ data/lang_unk_chain/L.fst -nt $langdir/L.fst ]; then
    ...
    cp -r $langdir data/lang_unk_chain
    silphonelist=$(cat data/lang_unk_chain/phones/silence.csl) || exit 1;
    nonsilphonelist=$(cat data/lang_unk_chain/phones/nonsilence.csl) || exit 1;
    # Use our special topology... note that later on may have to tune this
    # topology.
    steps/nnet3/chain/gen_topo.py $nonsilphonelist $silphonelist >data/lang_unk_chain/topo
  fi
fi
...
if [ $stage -le 15 ]; then
  ...
  steps/align_fmllr_lats.sh --nj 100 --cmd "$train_cmd" --generate-ali-from-lats true \
...
if [ $stage -le 16 ]; then
  ...
  steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
      ...
      --cmd "$train_cmd" 4000 ${lores_train_data_dir} data/lang_unk_chain $lat_dir $tree_dir
fi
...
if [ $stage -le 17 ]; then
  ...
  fast-lstmp-layer name=fastlstm1 cell-dim=$cell_dim recurrent-projection-dim=$projection_dim non-recurrent-projection-dim=$projection_dim delay=-3 $lstm_opts
  ...
  fast-lstmp-layer name=fastlstm2 cell-dim=$cell_dim recurrent-projection-dim=$projection_dim non-recurrent-projection-dim=$projection_dim delay=-3 $lstm_opts
  ...
  fast-lstmp-layer name=fastlstm3 cell-dim=$cell_dim recurrent-projection-dim=$projection_dim non-recurrent-projection-dim=$projection_dim delay=-3 $lstm_opts
...
if [ $stage -le 19 ]; then
  ...
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 data/langp_unk_test $dir $dir/graph
fi
