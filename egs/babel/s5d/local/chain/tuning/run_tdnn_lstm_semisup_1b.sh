...
sup_ali_dir=exp/tri5_cleaned_ali_train_cleaned_sp
...
affix=_semisup_1b  # affix for new chain-model directory trained on the combined supervised+unsupervised subsets
...
treedir=exp/chain${chain_affix}/tree

if [ $stage -le 14 ]; then
  utils/copy_data_dir.sh data/${unsupervised_set}_sp_hires data/${unsupervised_set}_sp
  steps/make_mfcc_pitch_online.sh --cmd "$train_cmd" --nj $nj \
    data/${unsupervised_set}_sp
  steps/compute_cmvn_stats.sh data/${unsupervised_set}_sp
  utils/fix_data_dir.sh data/${unsupervised_set}_sp
fi

if [ $stage -le 15 ]; then
  steps/nnet3/chain/build_tree_multiple_sources.sh --frame-subsampling-factor 3 \
      --context-opts "--context-width=2 --central-position=1" \
      --cmd "$train_cmd" --use-fmllr false 4000 data/lang_chain \
      data/${supervised_set}_sp $sup_ali_dir \
      data/${unsupervised_set}_sp $srcdir/best_path_${unsupervised_set}_sp \
      $treedir
fi
