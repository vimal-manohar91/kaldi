...
affix=_oracle_1a5  # affix for new chain-model directory trained on the combined supervised+unsupervised subsets
...
#            $graphdir data/${unsupervised_set}_sp_hires $unsup_lat_dir
#fi
...
unsup_lat_dir=exp/chain_cleaned/tri5_cleaned_${unsupervised_set}_sp_lats
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 17 ]; then
    ...
    $get_egs_script --cmd "$decode_cmd --h-rt 100:00:00" \
               --left-context $egs_left_context --right-context $egs_right_context \
               --left-context-initial $egs_left_context_initial --right-context-final $egs_right_context_final \
