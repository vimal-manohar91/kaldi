...
kl_decode_graph_affix=_300k_pp_ug
kl_decode_lang=data/lang_300k_pp_ug_test

unsup_lat_dir=$src_dir/decode${decode_graph_affix}_${unsupervised_set}  # training lattices directory
unsup_kl_lat_dir=$src_dir/decode${kl_decode_graph_affix}_${unsupervised_set}  # training lattices directory

norvb_unsup_lat_dir=$src_dir/decode${decode_graph_affix}_${norvb_unsupervised_set}  # training lattices directory
norvb_unsup_kl_lat_dir=$src_dir/decode${kl_decode_graph_affix}_${norvb_unsupervised_set}  # training lattices directory
...
if [ $stage -le 12 ]; then
  steps/best_path_weights.sh --cmd "$decode_cmd" \
    ${norvb_train_data_dir} ${norvb_unsup_lat_dir} \
...
kl_decode_graph_dir=$src_dir/graph${kl_decode_graph_affix}

if [ $stage -le 14 ]; then
  # Note: it might appear that this $lang directory is mismatched, and it is as
  # far as the 'topo' is concerned, but this script doesn't read the 'topo' from
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 ${kl_decode_lang} $src_dir $kl_decode_graph_dir
fi

if [ $stage -le 15 ]; then
  steps/nnet3/decode_semisup.sh --sub-split $nj --nj $nj --cmd "$decode_cmd" \
    --acwt 1.0 --post-decode-acwt 10.0 \
    --write-compact true --word-determinize false \
    --extra-left-context $extra_left_context \
    --extra-right-context $extra_right_context \
    --extra-left-context-initial 0 \
    --extra-right-context-final 0 \
    ...
    --online-ivector-dir $unsup_src_ivector_dir \
    --skip-scoring true \
    $kl_decode_graph_dir data/${norvb_unsupervised_set}_hires $norvb_unsup_kl_lat_dir || exit 1
fi

if [ $stage -le 16 ]; then
  utt_prefixes=
  for n in $(seq $num_data_reps); do
    utt_prefixes="$utt_prefixes rev${n}_"
  done

  local/semisup/copy_lat_dir.sh --write-compact true \
      --nj $nj --utt_prefixes "$utt_prefixes" \
      data/${unsupervised_set}_hires \
      ${norvb_unsup_kl_lat_dir} ${unsup_kl_lat_dir}

  ln -sf ../final.mdl $unsup_kl_lat_dir/final.mdl
fi
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 20 ]; then
    ...
    steps/nnet3/chain/get_egs_split.sh --cmd "$decode_cmd" --alignment-subsampling-factor 1 \
      ...
      --lattice-prune-beam 4.0 --kl-latdir $unsup_kl_lat_dir --kl-fst-scale $kl_fst_scale \
