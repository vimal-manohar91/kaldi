...
# This script is similar to _c, but updates existing teacher model instead 
...
kl_student_graph_affix=_pp
kl_student_lang=data/lang_pp_test
...
kl_lat_dir=$src_dir/decode${kl_student_graph_affix}_${tgt_dataset}_sp   # training lattices directory
...
kl_student_graph_dir=$src_dir/graph${kl_student_graph_affix}

if [ $stage -le 12 ]; then
  # Note: it might appear that this $lang directory is mismatched, and it is as
  # far as the 'topo' is concerned, but this script doesn't read the 'topo' from
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 ${kl_student_lang} $src_dir $kl_student_graph_dir
fi
...
if [ $stage -le 13 ]; then
  steps/nnet3/decode_semisup.sh --nj $nj --cmd "$decode_cmd" \
    ...
    --online-ivector-dir $teacher_ivector_dir \
    --skip-scoring true \
    $kl_student_graph_dir ${teacher_data_dir} $kl_lat_dir || exit 1
fi

if false && [ $stage -le 14 ]; then
...
egs_opts="$egs_opts --deriv-weights-scp $deriv_weights_scp --kl-latdir $kl_lat_dir --kl-fst-scale $kl_fst_scale"

if [ $stage -le 15 ]; then
  steps/nnet3/chain/train.py --stage $train_stage \
    --cmd "$decode_cmd" --trainer.input-model $dir/input.raw \
