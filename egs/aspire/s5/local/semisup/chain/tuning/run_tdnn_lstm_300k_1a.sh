...
lat_dir=${exp}/chain${chain_affix}/${gmm}_${rvb_train_set}_lats  # training lattices directory
dir=${exp}/chain${chain_affix}/tdnn_lstm${tdnn_affix}
train_data_dir=data/${rvb_train_set}_hires
train_ivector_dir=exp/nnet3/ivectors_train_rvb
...
norvb_lat_dir=${exp}/chain${chain_affix}/${gmm}_train_lats
...
if [ $stage -le 9 ]; then
  ...
  utils/split_data.sh data/${rvb_train_set} $nj

  for n in `seq $nj`; do
    awk '{print $1}' data/${rvb_train_set}/split$nj/$n/utt2spk | \
  ...
  $train_cmd JOB=1:$nj $lat_dir/log/copy_rvb_lattices.JOB.log \
    lattice-copy \
    "scp:utils/filter_scp.pl data/${rvb_train_set}/split$nj/JOB/utt2spk $lat_dir/lat_rvb.scp |" \
...
if [ $stage -le 11 ]; then
  ...
  steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
      ...
      --cmd "$train_cmd" 7000 data/${train_set} $lang ${exp}/${gmm} $treedir || exit 1
fi
