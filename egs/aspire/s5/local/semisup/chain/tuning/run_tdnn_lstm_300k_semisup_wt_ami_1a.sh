...
student_mfcc_config=conf/mfcc_hires.conf
...
tdnn_affix=_1b
chain_affix=_semisup_wt_ami_sdm1
nnet3_affix=_semisup_ts_ami_sdm1

kl_factor_schedule="output=0.5,0.5"
mmi_factor_schedule="output=0.5,0.5" 
...
# decode options
test_sets="ami_sdm1_dev ami_sdm1_eval"
...
if [ $stage -le 11 ]; then
  steps/best_path_weights.sh --cmd "$decode_cmd" \
    ${teacher_data_dir} $student_lang ${lat_dir} \
    $src_dir/best_path_${tgt_dataset}_sp
fi

ln -sf ../final.mdl ${lat_dir}

deriv_weights_scp=$src_dir/best_path_${tgt_dataset}_sp/weights.scp
...
egs_opts="--kl-fst-scale 0.5 --lattice-lm-scale 0.5 --lattice-prune-beam 4.0 --deriv-weights-scp $deriv_weights_scp --add-numerator-post true"
  ...
  for dset in $test_sets; do 
    (
      ...
      steps/nnet3/decode.sh --nj $nj --cmd "$decode_cmd" --config conf/decode.config \
        ...
        --frames-per-chunk 160 \
        --online-ivector-dir $(dirname $src_ivector_extractor)/ivectors_${dset} \
        --skip-scoring true \
