...
if [ -z "$sup_egs_dir" ]; then
  ...
  cp $src_dir/{den.fst,normalization.fst,tree,0.trans_mdl} $sup_egs_dir
  ...
  if [ $stage -le 13 ]; then
    ...
    steps/nnet3/chain/get_egs.sh --cmd "$decode_cmd" \
      ...
      data/${supervised_set}_16kHz_hires $sup_egs_dir $sup_lat_dir $sup_egs_dir
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 15 ]; then
    ...
    steps/nnet3/chain/get_egs_split.sh --cmd "$decode_cmd" --alignment-subsampling-factor 1 \
      ...
      $student_data_dir $unsup_egs_dir \
...
cp $sup_egs_dir/0.trans_mdl $dir

if [ $stage -le 19 ]; then
  steps/nnet3/chain/multilingual/train.py --stage $train_stage \
...
graph_dir=$treedir/graph${tgt_graph_affix}
if [ $stage -le 20 ]; then
  # Note: it might appear that this $lang directory is mismatched, and it is as
  # far as the 'topo' is concerned, but this script doesn't read the 'topo' from
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 ${tgt_lang} $treedir $graph_dir
fi

if [ $stage -le 21 ]; then
  rm -f $dir/.error
  nnet3-am-copy \
    --edits="remove-output-nodes name=output;rename-node old-name=output-1 new-name=output" \
    $dir/final.mdl $dir/final-output.mdl
