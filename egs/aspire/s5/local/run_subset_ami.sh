...
if [ $stage -le 6 ]; then
  mkdir -p data/ami_${mic}_train_reco12
  utils/filter_scp.pl -f 2 <(head -n 12 $ami_train/reco2file_and_channel) $ami_train/segments > data/ami_${mic}_train_reco12/uttlist
  utils/subset_data_dir.sh --utt-list data/ami_${mic}_train_reco12/uttlist $ami_train data/ami_${mic}_train_reco12
  #utils/copy_data_dir.sh $ami_train data/ami_${mic}_train_nreco12
  #utils/filter_scp.pl --exclude data/ami_${mic}_train_reco12/uttlist $ami_train/utt2spk > data/ami_${mic}_train_nreco12/utt2spk
  #utils/fix_data_dir.sh data/ami_${mic}_train_nreco12
fi

if [ $stage -le 7 ]; then
  local/semisup/chain/tuning/run_tdnn_lstm_300k_semisup_ts_ami_subset_1a.sh \
    ...
    --unsupervised-data-dir data/ami_${mic}_train \
    --supervised-data-dir data/ami_${mic}_train_reco12 \
    --chain-affix _semisup_ts_ami_subset_reco12_${mic} \
...
if [ $stage -le 8 ]; then
  local/semisup/chain/tuning/run_tdnn_lstm_300k_kl_ts_ami_subset_1a.sh \
    --src-dir exp/semisup300k/chain/tdnn_lstm_1b_sp \
    --treedir exp/semisup300k/chain/tree_bi_b \
    --src-ivector-extractor exp/nnet3/extractor \
    --unsupervised-data-dir data/ami_${mic}_train \
    --supervised-data-dir data/ami_${mic}_train_reco12 \
    --chain-affix _kl_ts_ami_subset_reco12_${mic} \
    --nnet3-affix _semisup_ts_ami_${mic} \
    --test-sets "ami_${mic}_dev_16kHz ami_${mic}_eval_16kHz" \
    --scoring-script $scoring_script \
