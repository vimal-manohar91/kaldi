...
rvb_data_dir=${adaptation_data_dir}_rvb_hires
...
if [ $stage -le 5 ]; then
  ...
  utils/combine_data.sh \
    ${rvb_data_dir} ${reverb_dirs} || exit 1
fi

if [ $stage -le 6 ]; then
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $rvb_data_dir/data/storage ]; then
    utils/create_split_dir.pl \
     /export/b0{5,6,7,8}/$USER/kaldi-data/egs/ami-$(date +'%m_%d_%H_%M')/s5/$rvb_data_dir/data/storage $rvb_data_dir/data/storage
  fi
...
rvb_ivector_dir=$src_ivector_root_dir/ivectors_${adaptation_set}_rvb
if [ $stage -le 7 ]; then
  steps/online/nnet2/extract_ivectors_online.sh \
    --cmd "$train_cmd" --nj $nj \
    $rvb_data_dir $src_ivector_root_dir/extractor \
    ${src_ivector_root_dir}/ivectors_${adaptation_set}_rvb || exit 1
fi

rvb_lat_dir=${sup_chain_dir}_lats_${adaptation_set}_rvb
if [ $stage -le 8 ]; then
  utt_prefixes=
  for name in noise music $maybe_babble; do 
  ...
  steps/copy_lat_dir.sh --cmd "$decode_cmd" --nj $nj --write-compact false \
    ...
    $rvb_data_dir $adaptation_lat_dir $rvb_lat_dir || exit 1
fi
