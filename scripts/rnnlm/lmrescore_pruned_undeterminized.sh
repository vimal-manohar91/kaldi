...
det_beam=8  # Beam used in determinization
...
$cmd JOB=1:$nj $outdir/log/determinize.JOB.log \
  lattice-determinize-pruned --acoustic-scale=$acwt --beam=$det_beam \
    "ark:gunzip -c $indir/lat.JOB.gz |" \
    "ark:gzip -c >$outdir/lat_tmp.JOB.gz" || exit 1
...
$cmd JOB=1:$nj $outdir/log/rescorelm.JOB.log \
  lattice-lmrescore-kaldi-rnnlm-pruned --lm-scale=$weight $special_symbol_opts \
    ...
    "ark:gunzip -c $outdir/lat_tmp.JOB.gz |"  ark:- \| \
  lattice-compose ark:- \
    "ark,s,cs:gunzip -c $outdir/lat_tmp.JOB.gz | lattice-scale --lm-scale=-1.0 ark:- ark:- | lattice-project ark:- ark:- |" ark:- \| \
  lattice-scale --acoustic-scale=0.0 ark:- ark:- \| \
  lattice-compose --write-compact=$write_compact \
    "ark:gunzip -c $indir/lat.JOB.gz |" ark,s,cs:- \
    "ark,t:|gzip -c>$outdir/lat.JOB.gz" || exit 1;

rm $outdir/lat_tmp.*.gz
