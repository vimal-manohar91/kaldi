...
void ClipGradientComponent::Read(std::istream &is, bool binary) {
  ...
  std::string token;
  ReadToken(is, binary, &token);
  if (token == "<L1NormBasedClipping>") { 
    ReadBasicType(is, binary, &l1_norm_based_clipping_);
    ReadToken(is, binary, &token);
  }
...
void ClipGradientComponent::Write(std::ostream &os, bool binary) const {
  ...
  WriteToken(os, binary, "<L1NormBasedClipping>"); 
  WriteBasicType(os, binary, l1_norm_based_clipping_);
...
std::string ClipGradientComponent::Info() const {
  ...
  stream << Type() << ", dim=" << dim_
         ...
         << (l1_norm_based_clipping_ ? "true" : "false")
...
void ClipGradientComponent::InitFromConfig(ConfigLine *cfl) {
  ...
  bool l1_norm_based_clipping = false; 
  ...
  cfl->GetValue("l1-norm-based-clipping", &l1_norm_based_clipping);
  if (!ok || cfl->HasUnusedValues() ||
      clipping_threshold < 0 || dim <= 0)
    KALDI_ERR << "Invalid initializer for layer of type "
              << Type() << ": \"" << cfl->WholeLine() << "\"";
  Init(dim, clipping_threshold, norm_based_clipping, l1_norm_based_clipping, 0, 0);
}
...
void ClipGradientComponent::Backprop(const std::string &debug_info,
  ...
  if (clipping_threshold_ > 0) {
    ...
    } else if (l1_norm_based_clipping_) {
      // each row in the derivative matrix, which corresponds to one sample in the 
      // minibatch, is scaled to have a max-l1-norm of clipping_threshold_
      CuVector<BaseFloat> clipping_scales(in_deriv->NumRows()),
        ones(in_deriv->NumCols());
      CuMatrix<BaseFloat> in_deriv_abs(*in_deriv);
      in_deriv_abs.ApplyPow(1.0);
      clipping_scales.AddMatVec(1.0 / clipping_threshold_, *in_deriv, kNoTrans,
                                ones, 0.0);
      // clipping_scales contains the l1 norm of each row divided by clipping_threshold
      int32 num_not_scaled = clipping_scales.ApplyFloor(1.0);
      if (num_not_scaled != clipping_scales.Dim()) {
        in_deriv->MulRowsVec(clipping_scales); 
        to_update->num_clipped_ += (clipping_scales.Dim() - num_not_scaled); 
...
ConvolutionComponent::ConvolutionComponent(
    ...
    max_param_change_(component.max_param_change_) {}
...
ConvolutionComponent::ConvolutionComponent(
    ...
    bias_params_(bias_params),
    max_param_change_(max_param_change) {
...
void ConvolutionComponent::Init(
    ...
    TensorVectorizationType input_vectorization,
    BaseFloat param_stddev, BaseFloat bias_stddev, 
    BaseFloat max_param_change, bool rand_init) {
  input_x_dim_ = input_x_dim;
  input_y_dim_ = input_y_dim;
  input_z_dim_ = input_z_dim;
  filt_x_dim_ = filt_x_dim;
...
std::string ConvolutionComponent::Info() const {
  ...
  stream << UpdatableComponent::Info()
         ...
         << ", max-param-change=" << max_param_change_;
...
void ConvolutionComponent::InitFromConfig(ConfigLine *cfl) {
  ...
  std::string matrix_filename;
  BaseFloat max_param_change = 0.0;
  ...
  cfl->GetValue("max-param-change", &max_param_change);
  if (cfl->GetValue("matrix", &matrix_filename)) {
    ...
    Init(input_x_dim, input_y_dim, input_z_dim,
         ...
         input_vectorization, param_stddev, bias_stddev, max_param_change, rand_init);
  }
  if (cfl->HasUnusedValues())
    KALDI_ERR << "Could not process these elements in initializer: "
	      << cfl->UnusedValues();
...
void ConvolutionComponent::Update(const std::string &debug_info,
  ...
  //
  // update
  // apply max-param-change for filter and bias gradients 
  BaseFloat scale = 1.0;
  if (max_param_change_ != 0.0) { 
    BaseFloat param_delta = learning_rate_ * std::sqrt(TraceMatMat(filters_grad, filters_grad, kNoTrans) +
      VecVec(bias_grad, bias_grad));
    if (param_delta > max_param_change_) {
      if (param_delta - param_delta != 0.0) {
        KALDI_WARN << "Infinite parameter change, will not apply.";
        filters_grad.SetZero();
      } else {
        scale *= max_param_change_ / param_delta;
        KALDI_LOG << "Parameter change for convolutional component is too big: " << param_delta << " > "
                  << "--max-param-change=" <<  max_param_change_
                  << ", scaling by " << max_param_change_ / param_delta;
      }
    }
  }
  filter_params_.AddMat(scale * learning_rate_, filters_grad);
