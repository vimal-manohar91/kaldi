...
void DeterminizeLattice(const fst::DeterminizeLatticePrunedOptions &opts, 
                        BaseFloat acoustic_scale,
                        BaseFloat beam,
                        Lattice *lat, CompactLattice *det_clat) {
...
int main(int argc, char *argv[]) {
  try {
    ...
    for (; !lattice_reader.Done(); lattice_reader.Next()) {
      ...
      if (lm_scale != 0.0) {
        DeterminizeLattice(opts, acoustic_scale, beam, &lat);
