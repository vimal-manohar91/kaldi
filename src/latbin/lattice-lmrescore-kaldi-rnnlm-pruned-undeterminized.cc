...
int main(int argc, char *argv[]) {
  try {
    ...
    fst::DeterminizeLatticePrunedOptions determinize_opts; // Options used in DeterminizeLatticePruned--
    ...
    po.Register("determinize-beam", &determinize_beam, 
                "Pruning beam [applied after acoustic scaling] for determinization.");
    ...
    // Reads and writes as lattice.
    SequentialLatticeReader lattice_reader(lats_rspecifier);
    LatticeWriter lattice_writer(lats_wspecifier);

    int32 num_done = 0, num_err = 0, num_warn = 0;
    ...
    for (; !lattice_reader.Done(); lattice_reader.Next()) {
      ...
      if (!DeterminizeLatticePruned(lat, determinize_beam, &det_clat, 
                                    determinize_opts)) {
      ...
      ComposeCompactLatticePruned(compose_opts, det_clat,
      ...
      if (composed_clat.NumStates() == 0) {
        ...
        Lattice tmp_lat;
        {
          ...
          fst::Project(&composed_lat, fst::PROJECT_OUTPUT);

          Lattice det_lat;
          fst::ConvertLattice(det_clat, &det_lat);
          fst::ScaleLattice(fst::GraphLatticeScale(-1.0), &det_lat);
          fst::Project(&det_lat, fst::PROJECT_OUTPUT);
          fst::ArcSort(&det_lat, fst::ILabelCompare<LatticeArc>());

          fst::Compose(composed_lat, det_lat, &tmp_lat);
        }

        fst::ArcSort(&tmp_lat, fst::ILabelCompare<LatticeArc>());

        Lattice lat_out;
        {
          Lattice lat;
          fst::ConvertLattice(det_clat, &lat);

          fst::Compose(lat, tmp_lat, &lat_out);
        }
        fst::TopSort(&lat_out);
