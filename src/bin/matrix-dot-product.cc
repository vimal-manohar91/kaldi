...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        " matrix-dot-product [options] <matrix-in-rspecifier1> [<matrix-in-rspecifier2>"
        " ...<matrix-in-rspecifierN>] <matrix-out-wspecifier>\n"
        "  e.g.: matrix-dot-product ark:1.weights ark:2.weights ark:combine.weights\n"
        " matrix-dot-product [options] <matrix-in-rxfilename1> [<matrix-in-rxfilename2>"
        " ...<matrix-in-rxfilenameN>] <matrix-out-wxfilename>\n"
        ...
        for (i = 0; i < N-2; ++i) {
          ...
          if (matrix_readers[i]->HasKey(key)) {
            ...
            if (SameDim(matrix2, matrix_out)) {
              ...
              if (matrix2.NumRows() - matrix_out.NumRows() <= length_tolerance) {
