...
BaseFloat PldaClusterable::Objf() const {
  ...
  KALDI_ASSERT(plda_->Dim() == stats_.Dim());
  
  for (size_t t = 0; t < dim; t++) {
    ans -= stats_(t) * stats_(t) 
           / (weight_ * weight_ * psi(t) + weight_);
    ans -= Log(1.0 + weight_ * psi(t));
    if (opts_.use_buggy_eqn)   // eqn(6) in the original paper
      ans += Log(weight_); 
...
BaseFloat PldaClusterable::Distance(const Clusterable &other_in) const {
  ...
  if (this->weight_ == 1 && other->weight_ == 1 && std::abs(llr) > 0.01) {
    ...
    KALDI_ASSERT(true || kaldi::ApproxEqual(llr, score));
  }
