// nnet2bin/nnet-convert-egs-discriminative-to-phone-or-pdf.cc
...
typedef TableWriter<KaldiObjectHolder<DiscriminativeNnetExamplePhoneOrPdf> >
DiscriminativeNnetExamplePhoneOrPdfWriter;
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:  nnet-convert-egs-discriminative-to-phone-or-pdf [options] <model-rxfilename> <egs-rspecifier> <egs-wspecifier1> [<egs-wspecifier2> ...]\n"
        "\n"
        "e.g.\n"
        "nnet-convert-egs-discriminative-to-phone-or-pdf 1.mdl ark:train.degs ark,t:text.degs\n"
        "or:\n"
        "nnet-convert-egs-discriminative-to-phone-or-pdf 1.mdl ark:train.degs ark:1.degs ark:2.degs\n";
        
    bool random = false;
    int32 srand_seed = 0;
    BaseFloat keep_proportion = 1.0;
    int32 const_feat_dim = 0;
    std::string phone_or_pdf = "pdf";
    ...
    po.Register("phone-or-pdf", &phone_or_pdf, "Specify pdf to convert to pdfs "
                "and phone to convert to phones");
    ...
    std::vector<DiscriminativeNnetExamplePhoneOrPdfWriter*> example_writers(num_outputs);
    for (int32 i = 0; i < num_outputs; i++)
      example_writers[i] = new DiscriminativeNnetExamplePhoneOrPdfWriter(
          po.GetArg(i+3));
      ...
      for (int32 c = 0; c < count; c++) {
        ...
        if (const_feat_dim == 0) {
          example_writers[index]->Write(ostr.str(),
              DiscriminativeNnetExamplePhoneOrPdf(example_reader.Value(), trans_model, phone_or_pdf) );
        } else {
          DiscriminativeNnetExamplePhoneOrPdf eg(example_reader.Value(), trans_model, phone_or_pdf);
