...
void SplitClusterToPoints(
  ...
  KALDI_ASSERT(pair_clusterable->clusterable1()->Type() == "plda");
  KALDI_ASSERT(pair_clusterable->clusterable2()->Type() == "adj");

  PldaClusterable *plda_clusterable = NULL;
  plda_clusterable = static_cast<PldaClusterable*>(
  ...
  for (std::set<int32>::const_iterator it = points.begin();
    ...
    split_clusterables->push_back(new PairClusterable(
          new PldaClusterable(plda_clusterable->plda(), pts, 
                              Vector<BaseFloat>(ivector_mat.Row(*it)), 
                              row_weights(*it)),
...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
      "Cluster ivectors using PLDA distance based on a greedy agglomerative "
      "clustering with search that accounts for the adjacency of segments.\n"
      "TODO better documentation\n"
      "Usage: agglomerative-cluster-plda-adjacency [options] "
      "<plda> <reco2utt-rspecifier> "
      "<ivector-rspecifier> <segmentation-rspecifier> "
      "<labels-wspecifier>\n"
      "e.g.: \n"
      " agglomerative-cluster-plda-adjacency plda ark:reco2utt \\\n"
      "   scp:ivectors.scp \"ark:segmentation-init-from-segments --shift-to-zero=false --frame-overlap=0.0 segments ark:- |\" \\"
      "   ark,t:labels.txt\n";
    ...
    std::string plda_rxfilename = po.GetArg(1),
      ...
      segmentation_rspecifier = po.GetArg(4),
      label_wspecifier = po.GetArg(5);
    ...
    for (; !reco2utt_reader.Done(); reco2utt_reader.Next()) {
      ...
      for (size_t i = 0; i < uttlist.size(); i++) {
        ...
        if (!ivector_reader.HasKey(utt)) {
          KALDI_WARN << "No iVector for utterance " << utt
      ...
      for (size_t i = 0; i < this_num_utts; i++) {
        ...
        if (compartment_size > 0) {
          ...
          clusterables[compartment].push_back(new PairClusterable(
                new PldaClusterable(&this_plda, points, 
