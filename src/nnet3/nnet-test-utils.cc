...
// A super-simple case that is just a single affine component, with TimeStretchCompoent, 
// to stretch input-layer.
void GenerateConfigSequenceTimeStretchSimplest(
    const NnetGenerationOptions &opts,
    std::vector<std::string> *configs) {
  std::ostringstream os;

  int32 input_dim = 100 + Rand() % 20,
       output_dim = 40 + Rand() % 10,
       affine_output_dim = 60 + Rand() % 20;

  os << "component name=stretch1 type=TimeStretchComponent input-dim="
     << input_dim << " output-dim=" << output_dim << std::endl;

  os << "component name=affine1 type=AffineComponent input-dim="
     << output_dim << " output-dim=" << affine_output_dim << std::endl;

  os << "input-node name=input dim=" << input_dim << std::endl;
  os << "component-node name=time1_node component=stretch1 input=input\n";
  os << "component-node name=affine1_node component=affine1 input=time1_node\n";
  os << "output-node name=output input=affine1_node\n";
  configs->push_back(os.str());
}
...
void GenerateConfigSequenceCnn2d(
  ...
  int32 input_vectorization = 2; //Rand() % 3;
  std::string vectorization;
  if (input_vectorization == 0) {
    vectorization = "yzx";
  } else if (input_vectorization == 1)  {
    vectorization = "zyx";
  } else {
    vectorization = "xyz";
  }
...
start:
  ...
  switch(network_type) {
    ...
    case 8:
      ...
      break;
    case 9:
      GenerateConfigSequenceTimeStretchSimplest(opts, configs);
      break;
    default:
