...
#include "nnet3/nnet-am-decodable-simple-multiple-outputs.h"
...
NnetDecodableBaseMultipleOutputs::NnetDecodableBaseMultipleOutputs(
  ...
  output_dims_.resize(output_nodes.size());
  log_priors_.resize(output_nodes.size());
  for (size_t i = 0; i < output_nodes.size(); i++) {
    output_dims_[i] = nnet_.OutputDim(output_nodes[i]);
    log_priors_[i] = priors[i];
    log_priors_[i].ApplyLog();
  }

  current_log_post_.resize(output_nodes.size());

  CheckAndFixConfigs();
}

int32 NnetDecodableBaseMultipleOutputs::GetIvectorDim() const {
  if (ivector_ != NULL)
    return ivector_->Dim();
...
void NnetDecodableBaseMultipleOutputs::EnsureFrameIsComputed(int32 subsampled_frame) {
  ...
  int32 current_subsampled_frames_computed = current_log_post_[0].NumRows(),
...
void NnetDecodableBaseMultipleOutputs::GetOutputForFrame(
                                          int32 output_index,
  ...
  output->CopyFromVec(current_log_post_[output_index].Row(
      subsampled_frame - current_log_post_subsampled_offset_));
}

void NnetDecodableBaseMultipleOutputs::GetCurrentIvector(int32 output_t_start,
...
void NnetDecodableBaseMultipleOutputs::DoNnetComputation(
    int32 input_t_start,
  ...
  request.outputs.resize(NumOutputs());
  for (size_t j = 0; j < NumOutputs(); j++) {
    IoSpecification output_spec;
    output_spec.name = output_nodes_[j];
    ...
    request.outputs[j].Swap(&output_spec);
  }
  ...
  for (size_t i = 0; i < NumOutputs(); i++) {
    ...
    computer.GetOutputDestructive(output_nodes_[i], &cu_output);
    // subtract log-prior (divide by prior)
    if (log_priors_[i].Dim() != 0)
      cu_output.AddVecToRows(-1.0, log_priors_[i]);
    // apply the acoustic scale
    cu_output.Scale(opts_.acoustic_scale);
    current_log_post_[i].Resize(0, 0);
    // the following statement just swaps the pointers if we're not using a GPU.
    cu_output.Swap(&(current_log_post_[i]));
    current_log_post_subsampled_offset_ = output_t_start / subsample;
  }
}

void NnetDecodableBaseMultipleOutputs::CheckAndFixConfigs() {
