...
/** Config structure for splitting discriminative
    training examples.
*/
...
void SplitDiscriminativeExample(
    ...
    std::vector<NnetDiscriminativeExample> *egs_out,
...
/** Remove unnecessary frames from discriminative training
    ...
    Uses "name" as the supervision object, whose lattice would be used 
    to determine if a particular frame is unnecessary.
*/
...
/**
   ...
   See also the documentation for MergeDiscriminativeExamples() which
   gives more details on how we append the examples.
*/
void CombineDiscriminativeExamples(
    int32 max_length,
    const std::vector<NnetDiscriminativeExample> &input,
    std::vector<NnetDiscriminativeExample> *output);
