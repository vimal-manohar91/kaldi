...
/*
void ArbitraryResample::Resample(const CuMatrixBase<BaseFloat> &input,
  ...
  CuVector<BaseFloat> output_col(output->NumRows());
  for (int32 i = 0; i < NumSamplesOut(); i++) {
    CuSubMatrix<BaseFloat> input_part(input, 0, input.NumRows(),
                                    first_index_[i],
                                    weights_[i].Dim());
    const CuVector<BaseFloat> &weight_vec(weights_[i]);
    output_col.AddMatVec(1.0, input_part,
                         kNoTrans, weight_vec, 0.0);
    output->CopyColFromVec(output_col, i);
  }
...
void ArbitraryResample::SetIndexes(const Vector<BaseFloat> &sample_points) {
  ...
  for (int32  i = 0; i < num_samples; i++) {
    ...
    if (index_max < 0)
      index_max = 1.0;
    if (index_min >= num_samples_in_)  
      index_min = num_samples_in_ - 2;
