...
void SetDerivWeightAndRenameOutput(BaseFloat weight,
  ...
  for (int32 i = 0; i < eg->outputs.size(); i++)
    if (eg->outputs[i].name == "output")
      if (weight != 0.0 && weight != 1.0)
        eg->outputs[i].deriv_weights.Scale(weight);
      ...
      if (frame_shift == 0 &&
        ...
        for (int32 c = 0; c < count; c++) {
          ...
          if (modify_eg_output) // Only for multilingual training
            SetDerivWeightAndRenameOutput(weight, new_output_name, &eg_modified_output);
        ...
        for (int32 c = 0; c < count; c++) {
          int32 index = (random ? Rand() : num_written) % num_outputs;
          if (modify_eg_output)
            SetDerivWeightAndRenameOutput(weight, new_output_name, &eg_out);
