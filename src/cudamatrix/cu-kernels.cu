...
template<typename Real>
__global__
static void _mul_col_groups_by_vec(Real* mat, const Real* scale, MatrixDim d,
                                   int group_size) {
  int32_cuda i = blockIdx.x * blockDim.x + threadIdx.x;
  int32_cuda j = blockIdx.y * blockDim.y + threadIdx.y;
  int32_cuda index = i + j * d.stride;
...
void cudaF_mul_col_groups_by_vec(dim3 Gr, dim3 Bl, float* mat, const float* scale,
                                 MatrixDim d, int group_size) {
  _mul_col_groups_by_vec<<<Gr,Bl>>>(mat,scale,d,group_size);
}
...
void cudaD_mul_col_groups_by_vec(dim3 Gr, dim3 Bl, double* mat, const double* scale,
                                 MatrixDim d, int group_size) {
  _mul_col_groups_by_vec<<<Gr,Bl>>>(mat,scale,d,group_size);
}
