...
void SplitInputSegmentation(const Segmentation &in_segmentation,
  ...
  for (SegmentList::const_iterator it = in_segmentation.Begin(); 
    ...
    int32 num_chunks = std::ceil(static_cast<BaseFloat>(length) / segment_length);
...
void SplitSegmentsUsingAlignment(int32 segment_length,
  ...
  for (SegmentList::iterator it = segmentation->Begin(); 
        it != segmentation->End();) {
    KALDI_ASSERT(segmentation->Dim() <= ali.size());  // Safety check. In practice, should never fail.
...
/** 
void SubSegmentUsingSmallOverlapSegments(
    const Segmentation &secondary_segmentation, int32 secondary_label,
    int32 subsegment_label, Segmentation *out_seg) {
    ...
    if (it->Label() == label) {
      ...
      if (it != segmentation->Begin()) {
        ...
        if (prev_it->end_frame >= it->end_frame) {
          ...
          it = segmentation->Erase(it);
          --it;   // TODO: This is buggy. So that we can increment in the for loop
...
bool GetClassCountsPerFrame(const Segmentation &segmentation,
